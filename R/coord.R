#' The coord class for storing annotated SVG (aSVG) instances
#'
#' The coord class is designed to represent annotated SVG (aSVG) instances. 
#' 
...
#' svgs <- parse_svg(svg.path=c(svg.pa1, svg.pa2), raster.path=NULL)
#'
...
#' svgs <- parse_svg(svg.path=c(svg.pa1, svg.pa2), raster.path=c(raster.pa1, raster.pa2))
#'
#' # Two aSVG instances are stored in a "coord" object of "svgs".
#' names(svgs)
...
#' # Combine two "coord" objects.
#' x <- svgs[1, ]; y <- svgs[2, ]; cmb(x, y)
#' # Extract slots from "svgs" and create a new "coord" object.
#' lis <- list(cordn=coordinate(svgs), attrb=attribute(svgs), svg=svg(svgs), raster=raster(svgs))
#' new.svgs <- coord(coordinate=lis$cordn, attribute=lis$attrb, svg=lis$svg, raster=lis$raster)
...
coord <- function(coordinate=list(), attribute=list(), dimension=list(), svg=list(), raster=list()) {
  ...
  check_coord(coord=coordinate, attr=attribute, wh=dimension, svg=svg, raster=raster)
  ...
  new('coord', coordinate=coordinate, attribute=attribute, dimension=dimension, svg=svg, raster=raster)
}
...
check_coord <- function(coord, attr, wh, svg, raster) {
...
#' @export
setMethod("show", c(object="coord"), function(object) {
  x <- object; nas <- names(x)
  if (length(nas) > 1) {
    na.pas <- paste(paste(nas[1:2], collapse=','), '...')
  } else na.pas <- nas
  message('Class "coord": ', 'instances(', length(x), ') - ', na.pas)
