...
# gg.lgd <- tar_cell_true(gg.lgd=gg.lis$g.lgd, gcol.lgd=gg.lis$gcol.lgd, blk.uni=blk.uni, profile=profile)
    ...
    if (is.sce) {
      ...
      dim.shm.lis <- dim_color_coclus(sce=sce.dimred, tar.bulk=tar.bulk, profile=profile, gg.dim = gg.dim, gg.shm.all=gg.all, grob.shm.all = grob.all, gg.lgd.all=lgd.all, col.shm.all = gcol.all, col.lgd.all=gcol.lgd.all, grob.lgd.all=grob.lgd.all, con.na=con.na, lis.match=NULL, sub.title.size=sub.title.size, dim.lgd.pos=dim.lgd.pos, dim.lgd.nrow=dim.lgd.nrow, dim.lgd.key.size=dim.lgd.key.size,  dim.lgd.text.size=dim.lgd.text.size, dim.capt.size=dim.capt.size)
      }
