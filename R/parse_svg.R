...
#'
#' Parse one or multiple aSVG files and store their coordinates and related attributes in a \code{coord} container, which will be used for creating spatial heatmap (SHM) plots.
...
#' @return An object of \code{coord} class, containing one or multiple aSVG instances. 

#' @seealso
#' \code{\link{coord}}: the \code{coord} class. 
...
parse_svg <- function(svg.path, raster.path=NULL, cores=1) {
    ...
    svg.all <- coord(coordinate=cordn, attribute=attrb, dimension=dimen, svg=svg, raster=raster); svg.all
}
