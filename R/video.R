...
video <- function(gg, cs.g, lgd, sam=NULL, covis.type=NULL, gcol.lgd=NULL, lgd.r=0.4, h=0.7, type='shm', lgd.title=NULL, sub.title.size=10, bar.width=0.12, bar.axis.title.size=5, bar.value.size=4, lgd.key.size=0.02, lgd.text.size=8, lgd.nrow.2nd=2, lgd.key.size.2nd=0.01, lgd.text.size.2nd=5, angle.text.key=NULL, position.text.key=NULL, lgd.row=2, lgd.row.2nd=2, lgd.col=NULL, legend.value.vdo=NULL, label=FALSE, label.size=4, label.angle=0, hjust=0, vjust=0, opacity=1, key=TRUE, dim.lgd.text.size=13, dim.lgd.key.size=4, dim.lgd.nrow=2, video.dim='640x480', res=500, interval=1, framerate=1, out.dir) {  
  # save(gg, cs.g, lgd, sam, covis.type, gcol.lgd, lgd.r, h, type, lgd.title, sub.title.size, bar.width, bar.value.size, lgd.key.size, lgd.text.size, lgd.key.size.2nd, lgd.text.size.2nd, angle.text.key, position.text.key, lgd.row, lgd.row.2nd, lgd.col, legend.value.vdo, label, label.size, label.angle, hjust, vjust, opacity, key, dim.lgd.text.size, dim.lgd.key.size, dim.lgd.nrow, video.dim, res, interval, framerate, out.dir, file='video.arg')
  ...
  output=file.path(dir, 'shm.mp4'), width=w.h[1], height=w.h[2], res=res, vfilter=paste0('framerate=fps=', framerate))
    ...
    for (i in na.dim) { # Refine dims.
      ...
      gg[[i]] <- ggplot(dat, aes(x=x, y=y, text=dat$feature)) + geom_point(size=1, alpha=0.7, colour=dat$fill, aes(colour=feature, shape=feature)) + theme_classic() + theme(plot.title=element_text(hjust=0.5, size=sub.title.size), legend.position='none', legend.text=element_text(size=dim.lgd.text.size), legend.background = element_rect(fill='transparent'), axis.text = element_blank(), axis.ticks = element_blank(), axis.title=element_text(size=4), axis.title.y = element_text(margin = margin(r=-0.2, unit='npc')), axis.title.x = element_text(margin = margin(t=0, unit='npc')), aspect.ratio=1) + scale_shape_manual(values=lgd.com$shape, breaks=lgd.com$br, guide=guide_legend(title=NULL, nrow=dim.lgd.nrow, override.aes = list(size=dim.lgd.key.size))) + labs(title=g0$labels$title, x=g0$labels$x, y=g0$labels$y, colour='', shape='')
    } 
