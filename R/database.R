...
check_se <- function(se) {
  form <- grepl('__', colnames(assay(se)))
  cdat <- colData(se); cna <- colnames(cdat)
  if (sum(form)==0) {
    if (!'spFeature' %in% cna) {
      msg <- paste0('One of the following requirements is required: 1) the colnames of assay slot follow the naming scheme "spatialFeature__variable"; 2) the "spFeature", "variable" (optional) columns are defined in the colData slot!')
      message(msg); return(msg)
    } else if (all(c('spFeature', 'variable') %in% cna)) { 
      colnames(se) <- paste0(cdat$spFeature, '__', cdat$variable)
    } else if ('spFeature' %in% cna & !'variable' %in% cna) {
      colnames(se) <- cdat$spFeature
    }
  }; return(se)
}
...
#' @param name One or multiple data set names (see the `data` argument in \link{write_hdf5}) in a vector, such as \code{c('test_leaf', 'test_chicken')}.  
