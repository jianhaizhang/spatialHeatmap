...
#' @param ds The module identification sensitivity. See function "adj.mod" for details.
...
#' \donttest{ matrix.heatmap("PSAC", exp, adj_mod, "2", "row") }
...
matrix.heatmap <- function(geneID, expr.table, adj.mod, ds, scale) {

   x <- x1 <- x2 <- y <- y1 <- y2 <- xend <- yend <- NULL 
   adj <- adj.mod[["adj"]]; mods <- adj.mod[["mod"]]

   # The last column of expr.table is expected to be annotation.
   col.len <- ncol(expr.table)
   if (!is.numeric(expr.table[, col.len])) gene <- expr.table[, -col.len] else gene <- expr.table
   lab <- mods[, ds][rownames(gene)==geneID]
    if (lab=="0") stop("The input gene is not assigned to any module. Please use a different one.")
