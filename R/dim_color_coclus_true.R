...
#'
#' True matching between cells and bulk tissues is included in \code{sce}.
#' 
...
dim_color_coclus_true <- function(sce=NULL, row.sel=NULL, tar.bulk=NULL, profile=FALSE, gg.dim, gg.shm.all, grob.shm.all, gg.lgd.all, col.shm.all, col.lgd.all, grob.lgd.all, con.na=TRUE, lis.match=NULL, sub.title.size=11, dim.lgd.pos='bottom', dim.lgd.nrow=1, dim.lgd.key.size=4, dim.lgd.text.size=13, dim.capt.size=13) {
 # save(sce, row.sel, tar.bulk, profile, gg.dim, gg.shm.all, grob.shm.all, gg.lgd.all, col.shm.all, col.lgd.all, grob.lgd.all, con.na, lis.match, sub.title.size, dim.lgd.pos, dim.lgd.nrow, dim.lgd.text.size, dim.lgd.key.size, dim.capt.size, file='dim.color.coclus.arg')
  ...
  if (profile==FALSE) dim.col <- col_dim_shm(gg.dim=gg.dim, gcol.all=col.lgd.all, lis.match=lis.match)
    ...
    df.all <- cbind(df.all, cdat[, c('cell', 'assignedBulk', 'response', 'dataBulk')])
...
tar_cell <- function(tar.bulk, df.all, row.sel=NULL) {  
  lis <- NULL; for (i in tar.bulk) {
    ...
    true.bulk <- strsplit(tar.all$dataBulk[1], ',|;| ')[[1]]
    true.bulk <- true.bulk[true.bulk!='']
    # True/false/other index. nrow(tar.all) == length(idx.true) + length(idx.other.tar). idx.false: non-target cells assigned target bulk.
    ...
    idx.other.tar <- which(df.all$dataBulk == tar.all$dataBulk[1] & df.all$response %in% c(FALSE, 'none'))
