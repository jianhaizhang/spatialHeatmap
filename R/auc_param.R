...
auc_param <- function(wk.dir, tar.par='norm', total.min=500, true.min=300, aucs=round(seq(0.5, 0.9, 0.1), 1)) { 
  ...
  # cell <- setdiff(nas, c('auc', 'dat', 'rds', unlist(par.lis))) # Has a bug in auc.knn.pca.fil1.fct.sc.mus.rds: sc and mus are treated two cell types. 
  ...
  frq_spd <- function(files=files, auc.min=auc.min, total.min=total.min, true.min=true.min, dir) { 
    ...
    if (!'spd.set' %in% colnames(df.all)) df.all$spd.set <- paste0('s', df.all$sim, 'p', df.all$sim.p, 'd', df.all$dim)
    df.frq <- df.all
    ...
    df.frq <- df.frq[, setdiff(colnames(df.frq), c('thr', 'sens', 'spec'))]
    cna <- colnames(df.frq)
