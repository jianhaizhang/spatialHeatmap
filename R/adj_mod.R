...
adj.mod <- function(data, type, minSize=15) {
    ...
    write.table(adj, paste0("./local_mode_result/", "adj.txt"), sep="\t", row.names=TRUE, col.names=TRUE)
    write.table(mcol, paste0("./local_mode_result/", "mod.txt"), sep="\t", row.names=TRUE, col.names=TRUE)
