...
#' @param dir The directory path where the folder "local_mode_result/" is created automatically to save adjacency matrix and module assignment as tab-sepatated "adj.txt" and "mod.txt" respectively. This parameter should be the same with that from the function \code{\link{filter_data}} so that the "processed_data.txt" generated by \code{\link{filter_data}} is saved in the same location. The default is NULL. This parameter is designed since the computation is intensive for large data matrix (e.g. > 10,000 genes). Therefore, to avoid repetitive computation when using the Shiny app launched by \code{\link{shiny_all}}, "adj.txt" and "mod.txt" can be saved and uploaded to "Compute locally" at a later time.  

#' @return A list containing the adjacency matrix and module assignment. If "dir" is not NULL, both are automatically saved in the directory "local_mode_result" as "adj.txt" and "mod.txt" respectively, which can be uploaded to \code{\link{shiny_all}}. 
