...
quantitative abundance levels of measured items, such as transcripts, proteins
...
identifying items with spatially selective abundance patterns (SE), groups of
items with related abundance profiles using matrix heatmaps combined with
...
functionality is described in a separate vignette called
...
file is defined by `svg=svg.hum`. The remaining arguments used here include:
...
shm.lis <- spatial_hm(svg=svg.hum, data=my_vec, ID='testing', ncol=1, height=0.9, width=0.8, sub.title.size=20, legend.nrow=2, ft.trans=c('g4320'))
```

The named numeric values in `my_vec`, that have name matches with the features in the 
chosen aSVG, are stored in the `mapped_feature` slot. The attributes of each the 
feature are stored in the `feature_attribute` slot.  
...
Take Figure \@ref(fig:humshm) as an example. The data feature `frontal.cortex' is re-matched to aSVG features `frontal.cortex` and `prefrontal.cortex`. The re-matching is defined in a list.  
...
Import of first png/svg image pair:  
```{r eval=TRUE, echo=TRUE, warnings=FALSE}
...
Import of second png/svg image pair:  
```{r eval=TRUE, echo=TRUE, warnings=FALSE}
...
Query the aSVG files with one element of the composite dimension, *i.e.* `embryo.0h.A`.   
...
The first two rows of the query results are displayed.  

```{r eval=TRUE, echo=TRUE, warnings=FALSE } 
feature.df[1:2, ]
```

The retrieved aSVG file `oryza.sativa_coleoptile.ANT_shm.svg` will be used for creating SHMs, and its path is accessed with `system.file`.      
...
```{r eval=TRUE, echo=TRUE, warnings=FALSE } 
unique(colData(rse.clp)$SFTimeCon) %in% unique(feature.df$feature)
```
...
where shared neighborhood information among items is used to preserve robust 
...
The information of brain-specific genes are also summarized in a table. Details of the talbe is explained in the [Spatial Enrichment](#se) section.    

```{r eval=TRUE, echo=TRUE, warnings=FALSE }
deg.tab.mus <- deg.all.mus$deg.table; deg.tab.mus[1:2, ] 
```

The numbers in `total` column are stringency measures of gene specificity, where the larger, the more strigent. For example, the up- and down-regulated genes in brain can be subsetted with the highest stringency `total==2`. Gene `ENSMUSG00000026764` (`Kif5c`) is selected as the reference gene for downstream analysis, since its function in synaptic transmission is well studied.     
...
svg.hum.pa <- system.file("extdata/shinyApp/example", 'homo_sapiens.brain.svg', package="spatialHeatmap") 
svg.hum.pa <- parse_svg(svg.hum.pa)
...
Query the aSVG files with an element of the composite dimension `embryo.0h`, and retrieve the file path of the target aSVG file `oryza.sativa_coleoptile.NT_shm.svg`.    
...
```{r eval=TRUE, echo=TRUE, warnings=FALSE } 
unique(target.clp$SFTime) %in% unique(feature.df$feature)
```
