...
First, SHM plots are generated for both the toy
...
imageA-1.png and imageA-1.svg, imageA-2.png and imageA-2.svg, etc. 
...
The SHM plots shown so far are restricted to the spatial dimension. This section
extends this to experiments with spatiotemporal resolution. A common use case for this are 
...
The data at single spatial dimension contains only two factors: samples and
conditions. By contrast, the spatiotemporal data contains three factors:
samples, conditions, and times (development stages). There are three
alternatives to organize the three factors: 1) combine samples and conditions;
2) combine samples and times; or 3) combine samples, conditions, and times.
...
one should achieve optimal visualization. In this example, the third is the
best choice and will be showcased in the first part. Meanwhile, for
demonstration purpose the second choice will also be illustrated in the second
part. In the spatiotemporal application, different development stages can be
represented in different aSVG images, and this feature will be presented in the
third part.   
...
For domenstration purpose, `brain` is selected as the target feature, `liver` and `kidney` as the reference features, and all the three strains `DBA.2J`, `C57BL.6`, `CD1` as the factors. 
...
The SE requires replicates in the count data and has build-in normalizing utilities, thus the pre-processing only involves filtering at this step.   

```{r eval=TRUE, echo=TRUE, warnings=FALSE }
data.sub.mus.fil <- filter_data(data=data.sub.mus, sam.factor='organism_part', con.factor='strain', pOA=c(0.2, 15), CV=c(0.8, 100))
```

The base methods in SE include four opitions: `edgeR`, `limma`, `DESeq2`, `distinct`, and the default are the first two. The count data are normalized by `TMM` method in `edgeR`. With each of the selected methods, the strains (`factors`) in each feature are treated as replicates and the target feature `brain` is compared with `liver` and `kidney` in a pairwise manner. The brain-specific genes are selected by log2 fold change (`log2.fc`) and FDR (`fdr`).   
...
This section showcases integrated discovery and visualization of potential novel genes in a particular biological process. Since network analysis/co-expression is involved, at least five spatial features (*e.g.* tissues) are required. Basically, gene abundance in target spatial feature is compared with other spatial features in a pairwise manner to obtain target-specific genes. Among these target-enriched genes, select a well characterized one as reference gene. Then network analysis is performed to detect gene modules. Genes comprising the same module are considered co-expressed and thus likely have similar functions. Therefore, in the module containing the reference gene, the unknown genes are candidates of novel genes having similar functions with the reference gene. As the procedures are explained in previous sections, this section will not explain them in details.   
...
The count data are normalized by `TMM` method in `edgeR`. Pairwise comparisons are conducted among the five tissues by `edgeR` and `limma`.   
...
Normalize the filtered count data with the same method (`TMM` in `edgeR`) as in `spatial_enrich`.  

```{r eval=TRUE, echo=TRUE, warnings=FALSE}
se.nor.enrich.mus <- norm_data(data=data.enrich.mus.fil, norm.fun='CNF', parameter.list=list(method='TMM'), log2.trans=TRUE)
```

`ENSMUSG00000026764` (`Kif5c`) is a mouse brain-specific gene and its function in synaptic transmission is well studied, so it is selected as reference gene.  

Subset normalized data with reference gene.   

```{r eval=TRUE, echo=TRUE, warnings=FALSE}
sub.mat.mus <- submatrix(data=se.nor.enrich.mus, ID=c('ENSMUSG00000026764'), p=0.1)
sub.mat.mus[1:2, ] # Subsetted assay matrix
```

Matrix heatmap of the subsetted data, where the reference gene `ENSMUSG00000026764` is indicated by black lines.   
...
Append annotation from `biomaRt`.  
...
ENSMUSG00000111977 (Gm47163) and ENSMUSG00000097960 have unclear functions. Since they are co-expressed with other genes in this module (Figure \@ref(fig:modMus)), they are potential novel genes responsible for synaptic signal transmission in mouse brain. Their expression profiles are visualized in Figure \@ref(fig:shmNovel) in form of SHMs and in Figure \@ref(fig:lineNovel) in form of line graph.   

```{r shmNovel, eval=TRUE, echo=TRUE, warnings=FALSE, fig.wide=TRUE, fig.cap=('SHMs of novel gene candidates in mouse brain synaptic transmission. Genes ENSMUSG00000111977 and ENSMUSG00000097960 are potential novel genes involved in mouse brain synaptic transmission, which are identified by `spatialHeatmap`. Their expression profiles are visualized in form of SHMs.'), out.width="100%", fig.show='show'}
spatial_hm(svg.path=svg.mus, data=deg.all.mus$deg.table, ID=c('ENSMUSG00000111977', 'ENSMUSG00000097960'), legend.r=1, legend.nrow=3, sub.title.size=9, ncol=3, bar.width=0.11)
```

```{r lineNovel, eval=TRUE, echo=TRUE, warnings=FALSE, fig.wide=TRUE, fig.cap=('Line graph of novel gene candidates in mouse brain synaptic transmission. The count data are normalized and replicates are aggregated.'), out.width="100%", fig.show='show'}
