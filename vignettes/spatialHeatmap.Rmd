...
library(spatialHeatmap); library(SummarizedExperiment); library(ExpressionAtlas); library(GEOquery); library(igraph); library(BiocParallel); library(kableExtra); library(org.Hs.eg.db); library(org.Mm.eg.db)
```
...
SHMs of the genes `'MTDHP3` and `YBX2` are organized
...
shm(data=dat.hum, ID=c('MTDHP3', 'YBX2'), lay.shm='con', legend.r=1.5, legend.nrow=3, h=0.6)
```
...
The following code saves the SHMs into an SVG file `shm_hum.svg` with the color scale and legend plot included.  
...
The following code colors spatial features in the original aSVG file with heat color from SHMs, and the resulting aSVG files are saved in `tmp.dir`.  

```{r eval=FALSE, echo=TRUE, warnings=FALSE, results='hide'}
color_svg(input=res, out.dir=tmp.dir)
```
...
shm(data=dat.mus, ID=c('H19'), legend.text.size=10, sub.title.size=9, ncol=3, legend.ncol=2, line.size=0.1, line.color='grey70')
```
...
```{r eval=TRUE, echo=TRUE, warnings=FALSE, collapse=TRUE}
dat.overlay <- read_fr(system.file('extdata/shinyApp/data/dat_overlay.txt', package='spatialHeatmap'))
dat.overlay[1:2, ]
```
...
dat.over <- SPHM(svg=svg.overlay, bulk=dat.overlay)
...
(*e.g.* organs, tissues) and treatments. In theory, the complexity of experimental designs scales 
to any number of variables in `spatialHeatmap`. This section
extends to experiments with three or more variables, such as experiments 
with spatiotemporal resolution and geographical locations, genotypes, treatments, *etc*.    

To visualize multi-variable spatial data, the variables are reduced to two by keeping the spatial feature and combining all other variables into a composite one. For instance, the following example contains four variables including spatial features, time points, drug treatments and injury conditions. The latter three are combined to produce a composite variable.   
...
shm(data=dat.mul.svg, ID=c('gene1'), width=0.7, legend.r=0.2, legend.width=1, preserve.scale=TRUE, bar.width=0.09, line.color='grey50') 
```
...
The Spatial Enrichment (SpEn) functionality is an extension of the SHMs to identify groups of
transcripts, proteins or other biomolecules that are particularly abundant or
enriched in certain spatial regions, such as tissue-specific transcripts. Given a group of spatial features, it compares the abundance profiles of biomolecules between each spatial feature and all other features. The biomolecules significantly up- or down-regulated in one feature relative to all other features are denoted spatially enriched or depleted biomolecules respectively. Then by querying a feature in the enrichment results, the corresponding depleted and enriched biomolecules will be returned. The methods for detecting differentially-expressed genes (DEGs) include edgeR [@edgeR], limma [@limma], DESeq2 [@deseq2], distinct [@distinct]. In a simliar manner, the SpEn is also able to identify biomolecules specifically expressed or depleted in a certain treatment.   

The application of SpEn is illustrated here using the 
above [mouse organ data](#mus) as example. The function `sf_var` is developed to subset the spatial features and variables in the assay data. In the following, five features ('brain', 'liver', 'lung', 'colon', and 'kidney') and three variables (mouse strains 'DBA.2J', 'C57BL.6', and 'CD1') are subsetted. The `com.by` argument specifies the enrichment will be performed on spatial features (`ft`) or variables (`var`). Here `com.by` is set `ft` and the variables will be treated as replicates in the differential expression analysis.    
...
The SpEn is implemented in function `spatial_enrich`. In the following, the method `edgeR` is chosen and the count data are internally normalized by the `TMM` method from `edgeR`. The enrichment results are selected by log2 fold change (`log2.fc`) and FDR (`fdr`) thresholds. The `outliers` argument is designed to specify number of outlier features allowed.   

```{r eval=TRUE, echo=TRUE, warnings=FALSE, results='hide'}
enr.res <- spatial_enrich(sub.mus.fil, method=c('edgeR'), norm='TMM', log2.fc=1, fdr=0.05, outliers=1)
```

The overlaps of enriched biomolecules (`type='up'`) across features are presented in an UpSet plot (`plot='upset'`). Assigning `matrix` or `venn` to the `plot` argument will present the overlaps in form of a matrix plot or Venn diagram respectively.     

```{r enOvl, eval=TRUE, echo=TRUE, warnings=FALSE, fig.wide=TRUE, fig.cap=('Overlap of enriched biomolecules across features.'), out.width="100%", fig.show='show', results='hide'}
ovl_enrich(enr.res, type='up', plot='upset')
```

The enriched and depleted genes detected by `edgeR` in brain can be accessed as follows. The `type` column refers to up- or down-regulated while the `total` column shows the total reference features.   
...
(`Cdhr5`) gene in mouse brain are chosen to plot SHMs. The resulting SHMs are termed as SHMs of spatially-enriched/depleted biomolecules (transcripts).   
...
network(ID="Kif5c", data=sub.mat, adj.mod=adj.mod, adj.min=0, vertex.label.cex=1.2, vertex.cex=2, static=TRUE)
```   
...
1. Landing Page: gallery of pre-upload examples, menu for uploading custom data sets, selecting default dat sets, or downloading example data sets.    
2. Spatial Heatmap: interactive table of assay data, SHMs created with single or multiple genes and treatments, settings to customize the SHMs, data mining (hierarchical clustering, K-means clustering, network analysis).  
3. Spatial Enrichment: settings, overlap plots of enrichment results across spatial features, data table of enriched and depleted biomolecules.  
4. Co-visualization: options for cell grouping (annotation, clustering, manual, automatic), options for mapping direction (cell-to-bulk, bulk-to-cell), results of automated co-clustering, interactive inferface for matching cell groups and tissues in aSVGs.   
5. Settings Control Menu: postioned on top of each panel.  

A screenshot is shown below depicting an SHM plot generated with the `spatialHeatmap` Shiny App (Figure \@ref(fig:shiny)).   
...
After launching, the Shiny App displays by default one of the pre-included data sets.   

The assay data and aSVG images are uploaded to the Shiny App as tabular (_e.g._ in CSV or TSV format) and SVG files, respectively. To also allow users
...
the user specifies the output file name under the `file` argument. This will create a tablular file in TSV format. 
...
custom data and aSVGs and define default settings within most visualization
panels (*e.g.* color schemes). For details users want
to consult the help file of the `custom_shiny` function. To maximize
flexibility, the default settings are stored in a yaml file in the Shiny App.
This makes it easy to refine and optimize default settings simply by changing
this yaml file.

## Database Backend

To maintain scalability, the customized Shiny App is designed to work with HDF5-based database [@rhdf5; @hdf5array], which enables users to incorporate data and aSVGs in a batch. The database is constructed with the function `write_hdf5`. Basically, the accepted data formats are `data.frame` or `SummarizedExperiment`. Each data set is saved in an independent HDF5 database. Meanwhile, a pairing table describing matchings between data and aSVGs is required. All individual databases and the pairing table are then compressed into a file "data_shm.tar". Accordingly, all aSVG files should be compressed in another tar file such as "aSVGs.tar". Finally, these two tar files are included in the Shiny App by feeding their paths in a list to `custom_shiny`.   
...
`shm` function call in the code block below is not evaluated. Thus,
...
biomolecule name, such as a gene ID. The naming of spatial features and conditions in the 
...
the target file contains at least two columns: one for the spatial features and
...
the `assay` slot, where the rows correspond to biomolecules, such as gene IDs.
...
2. It should contain at least two columns. One column represents spatial features
   ...
   corresponding features entry.
 
3. The feature names must have matching entries in corresponding aSVG
   ...
   should be avoided for naming features and conditions. 
...
SHMs supports re-matching between data and aSVG features, *i.e.* re-match one spatial feature in the data to a different or multiple counterparts in the aSVG. The re-matching is defined in a named list, where a name slot refers to a spatial feature in data and the corresponding elements in the list are aSVG features for re-matching.   

The example below takes data from the [Human Brain](#hum) section, such as `svg.hum.sub` and `se.fil.hum`. Figure \@ref(fig:shmNoMatch) is the SHM before re-matching. The data feature `frontal.cortex` is re-matched to aSVG features `frontal.cortex` and `prefrontal.cortex` in a list (`remat.hum`), and Figure \@ref(fig:remat) is the SHM after re-matching.  
...
shm(data=dat.no.match, ID=c('MTDHP3'), lay.shm='con', ncol=1, legend.r=0.8, legend.nrow=2, h=0.6)
```

```{r remat, eval=TRUE, echo=TRUE, warnings=FALSE, fig.wide=TRUE, fig.cap=("SHMs after re-matching. The data feature `frontal.cortex` is re-matched to aSVG features `frontal.cortex` and `prefrontal.cortex`."), out.width="100%", results='hide'}
dat.rematch <- SPHM(svg=svg.hum.sub, bulk=se.fil.hum, match=remat.hum)
shm(data=dat.rematch, ID=c('MTDHP3'), lay.shm='con', ncol=1, legend.r=0.8, legend.nrow=2, h=0.6)
```

Examples of re-matching application include mapping gene expression profiles of one species to a closely related species in the same anatomical region, or mapping gene assay profiles of a sub-tissue to the whole tissue, where the gene abundance profiles of the whole tissue are hard to assay and only a sub-tissue is assayed.     
