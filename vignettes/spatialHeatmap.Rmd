...
-->
```{r eval=FALSE, echo=TRUE, warnings=FALSE }
tmp.dir <- paste0(tempdir(check=TRUE), '/shm')  # Create empty directory
...
-->
```{r eval=FALSE, echo=TRUE, warnings=FALSE }
tmp.dir <- paste0(tempdir(check=TRUE), '/shm')
...
-->
```{r eval=FALSE, echo=TRUE, warnings=FALSE }
tmp.dir1 <- paste0(tempdir(check=TRUE), '/shm1')
svg.hum <- system.file("extdata/shinyApp/example", 'homo_sapiens.brain.svg', package="spatialHeatmap") 
file.copy(from=svg.hum, to=tmp.dir1, overwrite=FALSE) # Copy "homo_sapiens.brain.svg" file into 'tmp.dir1'
```
...
```{r eval=FALSE, echo=TRUE, warnings=FALSE }
feature.df <- return_feature(feature=c('frontal cortex'), species=c('homo sapiens', 'brain'), dir=tmp.dir1, remote=FALSE, keywords.any=FALSE)
feature.df
``` 
