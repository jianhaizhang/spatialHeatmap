...
can be addressed and colored programmatically. One or multiple aSVG files can be parsed automatically by utilities in `spatialHeatmap` and stored in an S4 class `coord` (Figure \@ref(fig:illus)B). The slots of `coord` include `coordinate`, `attribute`, `dimension`, `svg`, and `raster`, corresponding to feature coordinates, styling attributes (color, line width, etc.), width and heigth, aSVG file paths, and raster image paths respectively. The raster images are only required when they are superimposed with SHMs (Figure \@ref(fig:overCol)).   
...
The following example generates a small numeric testing vector, where the data slot
contains four numbers and its name slot is populated with three "lobe" feature
names selected from the above aSVG instance (`feature.df.hum`). In addition, a non-matching entry
