...
-->
<a name='hum_single'></a>
```{r humshm, eval=TRUE, echo=TRUE, warnings=FALSE, fig.wide=TRUE, fig.cap=("SHM of human brain. Only cerebellum and frontal cortex are colored, because they are present in both the aSVG and the expression data. The legend plot on the right maps the feature labels to the corresponding spatial regions in the image."), out.width="100%", fig.show='hide' }
...
This image is hidden to meet the 5MB requirement on vignette.
-->
```{r musshm1, eval=TRUE, echo=TRUE, warnings=FALSE, fig.wide=TRUE, fig.cap=("SHM of mouse organs. This is a multiple-layer image where the view onto 'lung' and 'heart' is obstructed by displaying the 'skeletal muscle' tissue."), out.width="100%", fig.show='hide' }
spatial_hm(svg.path=svg.mus, data=se.fil.mus, ID=c('ENSMUSG00000000263'), height=0.6, legend.text.size=10, sub.title.size=9, ncol=3, legend.ncol=2, line.size=0.2, line.color='grey70', ft.trans='path4204')
```
...
```{r eval=FALSE, echo=TRUE, warnings=FALSE } 
    inter1 <- function(env1=parent.frame()) {
...
test <- function(x, y) {
    print(vapply(match.call(), deparse, character(1))[-1])
    print(deparse(match.call()))
    print(names(deparse(match.call())))
    z <- y+1;
 #print(inter(z=z))
    #cat(ls(), '\n')
}
...
# Copy doc. In the copy all children of out and ply are deleted and each child is inserted back to out or ply to extract coordinates. The benefit is if a child has no coordinates in ps.xml, this child is detected and it does not count in tis.path, so length(tis.path)==length(df$tissue) is secured. The drawback is time-consuming.
doc0 <- xml_new_root(doc, .copy=TRUE)
...
out.ply0 <- out_ply(doc0)
out0 <- out.ply0$out; ply0 <- out.ply0$ply
chdn.out0 <- out.ply0$chdn.out; chdn.ply0 <- out.ply0$chdn.ply
chdn.all0 <- c(chdn.out0, chdn.ply0)
id.all0 <- out.ply0$id.all; na.all0 <- out.ply0$na.all

df.out0 <- cord(doc0, out0, ply0, out0)
df.ply0 <- cord(doc0, out0, ply0, ply0)



df.out <- cord_parent(svg.path, 'out', feature=NULL)
df.ply <- cord_parent(svg.path, 'ply', feature=NULL)

update_use(chdn.all, id.all, ply, chdn.ply)
write_xml(doc, 'shm_up.svg')

rsvg_ps('test.svg', 'test.svg.ps')
PostScriptTrace('/home/jzhan067/spatialHeatmap/vignettes/test.svg.ps', '/home/jzhan067/spatialHeatmap/vignettes/test.svg.ps.xml')

library(ggplot2)
ggplot(df.ply$df, aes(x = x, y = y)) + geom_polygon(aes(fill=NULL, group =tissue))

svg.path <- '~/anatomogram/src/svg/homo_sapiens.male.svg'
library(xml2); library(rsvg); library(grImport)
source('../R/path_br_all.R'); source('../R/path_br.R')
source('../R/svg_attr.R')
...
tran.mat1 <- xml_attr(chdn.out, 'id')[grep('^matrix', xml_attr(chdn.out, 'transform'))]
tran.mat2 <- xml_attr(chdn.ply, 'id')[grep('^matrix', xml_attr(chdn.ply, 'transform'))]
xml_attr(out, 'transform'), 

for (i in unique(tis.path)) {
    if (nrow(subset(df, grepl(paste0('^', i), tissue)))!=nrow(subset(df.all, grepl(paste0('^', i), tissue)))) print(i)
}

for (i in unique(tis.path)) {
    sub <- subset(df, grepl(paste0('^', i), tissue))
    sub.all <- subset(df.all, grepl(paste0('^', i), tissue))
    if (!identical(sub, sub.all)) print(i)
}
