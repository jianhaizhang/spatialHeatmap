...
```{r eval=TRUE, echo=TRUE, warnings=FALSE}
target.clp[1:2, c('age', 'organism_part', 'stimulus', 'con')] # A slice of the targets file.
unique(target.clp[, 'age']) # All development stages.
unique(target.clp[, 'organism_part']) # All tissues.
unique(target.clp[, 'con']) # All conditions.
```

The three dimensions are combined using the convenience function `com_factor`, which is the main difference with the previous examples involving only two dimensions. The targets file including the new composite dimension (`SFTimeCon`) is loaded to the `colData` slot in `rse.clp` internally.   

```{r eval=TRUE, echo=TRUE, warnings=FALSE}
rse.clp <- com_factor(rse.clp, target.clp, factors2com=c('organism_part', 'age', 'con'), sep='.', factor.new='SFTimeCon')
colData(rse.clp)[1:2, c('age', 'organism_part', 'stimulus', 'con', 'SFTimeCon')]
unique(colData(rse.clp)$SFTimeCon) # All elements of the composite dimension.  
```
...
Elements of the three dimensions are shown: spatial features, times, conditions.   

```{r eval=TRUE, echo=TRUE, warnings=FALSE}
target.clp[1:3, c('age', 'organism_part', 'stimulus', 'con')] # A slice of the targets file.
unique(target.clp[, 'age']) # All development stages.
unique(target.clp[, 'organism_part']) # All tissues.
...
target.clp[1:3, c('age', 'organism_part', 'stimulus', 'con', 'SFTime')]
```
...
se.fil.clp2 <- filter_data(data=se.aggr.clp2, sam.factor='SFTime', con.factor='stimulus', pOA=c(0.07, 7), CV=c(0.7, 100), dir=NULL) # Filtering of genes with low counts and varince.
```
