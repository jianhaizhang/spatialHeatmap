...
spatial_hm(svg.path=svg.hum, data=se.fil.hum, ID=c('ENSG00000268433', 'ENSG00000006047'), lay.shm='con', width=0.8, height=1, legend.r=1.5, legend.nrow=2, out.dir=tmp.dir)
```
...
spatial_hm(svg.path=svg.mus, data=se.fil.mus, ID=c('ENSMUSG00000000263'), height=0.7, legend.width=0.7, legend.text.size=10, sub.title.size=9, ncol=3, tis.trans=c('skeletal muscle'), legend.nrow=4, line.size=0.2, line.color='grey70')
```
...
```{r eval=FALSE, echo=TRUE, warnings=FALSE } 
# Draft of the spatial-temporal data: https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE115373

rse.clp <- getAtlasData('E-GEOD-115371')[[1]][[1]]

cdat <- colData(rse.clp)

sub_tar <- function(df.tar, column, old, new, sub.row) {
...
row <- cdat$stimulus==cdat[46, 'stimulus']
old <- '72 hour anaerobic environment; 24 hour aerobic environment'
new <- '72N24A'
tar <- sub_tar(cdat, 'stimulus', '72 hour anaerobic environment; 24 hour aerobic environment', '72N24A')
tar <- as.data.frame(tar)
unique(tar$stimulus)

tar <- sub_tar(tar, 'stimulus', 'anaerobic environment', 'anaerobic')
unique(tar$stimulus)
tar <- sub_tar(tar, 'stimulus', 'none', 'aerobic')
unique(tar$stimulus)

tar <- sub_tar(tar, 'age', unique(tar$age), c('0h', '1h', '3h', '12h', '24h', '48h', '72h', '96h'))
unique(tar$age)

tar <- sub_tar(tar, 'organism_part', unique(tar$organism_part), c('embryo', 'embryoColeoptile', 'coleoptile'))
unique(tar$organism_part)

as.data.frame(tar[, c('organism_part', 'age', 'stimulus')])
tar$samTime <- paste0(tar$organism_part, tar$age) 

tar <- sub_tar(tar, 'samTime', 'coleoptile96h', 'coleoptile72N24A', tar$stimulus=='72N24A')
unique(tar$samTime)

as.data.frame(tar[, c('organism_part', 'age', 'stimulus', 'samTime')])

tar$con <- tar$stimulus
tar <- sub_tar(tar, 'con', c('aerobic', 'anaerobic'), c('A', 'N'))
unique(tar$con)
tar$samTimeCon <- paste0(tar$samTime, tar$con)
tar <- sub_tar(tar, 'samTimeCon', c('coleoptile72N24A72N24A'), c('coleoptile72N24A'))
...
se.fil.clp <- filter_data(data=se.aggr.clp, sam.factor='samTimeCon', con.factor=NULL, pOA=c(0.06, 7), CV=c(0.7, 100), dir=NULL)

svg.clp.ant <- '~/SVG_tutorial_file/svg_repo/coleoptile/coleoptile_ANT.svg'
shm.df <- spatial_hm(svg.path=svg.clp.ant, data=se.fil.clp, ID=c('Os12g0630200', 'Os12g0628600'), legend.r=1, legend.key.size=0.02, legend.text.size=12, legend.nrow=16, ncol=1)

svg.clp.n <- '~/SVG_tutorial_file/svg_repo/coleoptile/coleoptile_N.svg'
shm.df <- spatial_hm(svg.path=svg.clp.n, data=se.fil.clp, ID=c('Os12g0630200'), height=0.7, legend.r=1.5, legend.key.size=0.02, legend.text.size=12, legend.nrow=5)

svg.clp.an <- '~/SVG_tutorial_file/svg_repo/coleoptile/coleoptile_AN.svg'
shm.df <- spatial_hm(svg.path=svg.clp.an, data=se.fil.clp, ID=c('Os12g0630200'), height=0.7, legend.r=1.5, legend.key.size=0.02, legend.text.size=12, legend.nrow=5, line.color='black')
...
doc <- read_xml(svg.path); spa <- xml_attr(doc, 'space')
len <- xml_length(doc); out <- xml_children(doc)[[len-1]]; node.parent <- ply <- xml_children(doc)[[len]]
 ...
 len <- xml_length(node.parent); chdn <- xml_children(node.parent)
  for (i in seq_len(len)) {
    ...
    nod0 <- chdn[[i]]; na <- xml_name(nod0); id <- xml_attr(nod0, 'id')
      ...
      if (!na %in% c('g', 'use', 'title', 'ellipse', 'rect')) { dot <- rm_dot(nod0); if (dot=='yes') next }
      ...
      if (na!='g') path_br(nod0, g=TRUE) else {
        ...
        for (j in seq_along(nod0.chl)) {
          ...
          nod1 <- nod0.chl[[j]]; if (!xml_name(nod1) %in% c('title', 'ellipse', 'rect')) dot <- rm_dot(nod1)
...
```{r eval=FALSE, echo=TRUE, warnings=FALSE } 
f.new <- c('prefrontal.cortex', 'frontal.cortex')
```
...
Finally, the features are updated in the aSVG file(s) located in the `tmp.dir1` directory.
