...
corresponding spatial features (celllular compartments, tissues, organs, *etc*) defined in anatomical images according to a
...
_spatial heatmap_ (SHM) plot. <font style='color:red'> The SHM has extended functionalities of spatial enrichment (SE) and clustering. SE is specialized in detecting genes that are specifically expressed in a particular spatial feature, while clustering is designed to detect biological molecule groups sharing related abundance profiles (_e.g._ gene modules) and visualize them in matrix heatmaps combined with hierarchical clustering dendrograms and network representations. Moreover, a novel functionality of integrated co-visualization of bulk tissues and single cells is also developed. Sinlge cells and bulk tissues are co-visualized in embedding plots using PCA, UMAP, or TSNE and SHM plots respectively, which form a composite plot. The co-visualization is detailed in the other vignette.  </font>     
...
packages dealing with gene-, protein- and metabolite-level profiling data. In case of gene expression data, the `assays` slot of
...
functionalities of SHM, SE, and clustering in the `spatialHeatmap` package using as initial example a simple
to understand toy data set, and then more complex mRNA profiling data from the
Expression Atlas and GEO databases. The co-visualization is explained in a separate vignette. </font> First, SHM plots are generated for both the toy
...
library(igraph); library(BiocParallel); library(biomaRt)
```
