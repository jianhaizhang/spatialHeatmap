...
Jianhai: explanations on red color: <br>
1. Any changes (including small changes) are indicated by red color. <br>  
2. A paragraph in red: either big changes in text or relocated without text change.  
</font>
...
color and line width information, respectively. <font style='color:red'> The `feature` column includes group labels for sub-features in the `sub.feature` column. SHM plots are created by mapping assay data to labels in `feature`. </font> 
...
<font style='color:red'>
JH: First example on real data: since it includes all necessary explanations, its content is relatively long. While the subsequent examples are shorter as a result of referring to the first one. 
</font> 
...
<font style='color:magenta'>I have tried to shorten the above and leave content where
I can roughly follow. I am still lost (and most readers will be) what you mean by 
'condition' (usually they are treatments?) and what it means to combine conditions. Does
this mean you are creating summary stats such as mean/median? If so what's the purpose
of having additional conditions if you erase/combine them. Probably you mean something 
else but I don't understand what you mean?
</font>
...
I would stick to SHM only since the time variable is only of many other possible ones.
I doubt you want to introduce acronyms for all of them. Just say SHM including time or stress.
If useful you could introduce a variable acronym syntax such as SHM_time, SHM_stress
or SHM_time_stress, etc.
</font>
...
<font style='color:red'> The example below takes assay data from the [Human Brain](#hum) section, such as `svg.hum.sub` and `se.fil.hum`. The spatial feature `frontal.cortex` in assay data is re-matched to `frontal.cortex` and `prefrontal.cortex` in the aSVG through a `list` (`remat.hum`). Figure \@ref(fig:shmNoMatch) is the SHM before re-matching while Figure \@ref(fig:remat) is the SHM after re-matching.   
</font>
