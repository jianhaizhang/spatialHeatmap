...
assignments. The following introduces the three steps of this method based 
on RNA-Seq data.

1. The raw read count matrix of the single cell data is reduced to those genes
   ...
   matrix. As a result the sparcity in the single cell data has been largely removed
...
2. The combined single cell and bulk read count data are normalized jointly.
   Next, the preprocessed and normalized single cell and bulk data are
   separted. Only the single cell data is filtered a second time. Embedding 
   is performed using PCA (UMAP or other).
3. Co-clustering is performed on the embedding data where single cells are joined
   with the nearest neighbor bulk tissu. 
