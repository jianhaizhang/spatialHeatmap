...
the user can choose among manual, annotation and automatic methods. They can
be used from R as well as the corresponding Shiny app [@shiny, @shinydashboard].   
...
SCE objects contain three additional slots that are used by the 
co-visualization functionality of this package. First, SCE has a `label` column 
in the `colData` slot for storing [cell type annotation labels](#cusLab). <span style="color: red;">[ThG: please revise 
...
be removed since it is too detailed for an intro.]</span> Second,
when using the Shiny app, single cell data in manual matching or combined
single cell and bulk data in auto-matching are saved in an `.rds` file of
`SingleCellExperiment` object by `saveRDS`. Third, when using the auto-matching
in Shiny app, the [true matching table](#autoSCE) across single cells, bulk
tissues, and aSVG features is stored in the `metadata` slot of SCE.     

The images in co-visualization are in the same form of annotated SVGs (aSVGs)
with SHM plots, which is detained in the other vignette.   
...
co-visualizing, single cell data are pre-processed and clustered, which is
...
seed for Random Number Generator at the beginning, which is required only once
in each R session.     
...
Clustering. Cell clusters are detected by first building a graph object then
...
The custom cell clusters are defined in the `label` column of `colData`, which
are cell sources provided in the original study.  
