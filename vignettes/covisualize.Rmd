...
example of a cluster file is included  in the `spatialHeatmap` package. In this case the group labels were created by the `cluster_cell` function. The details of this function are available in its help file. The cluster file contains at least two columns: a `cell` column containing single cell identifiers used under `colData` and a `cluster` column with the
cell group labels.   
...
An Embedding plot of single cells is created. The cells are colored by manual groups in the `cluster` column in `colData`.   
