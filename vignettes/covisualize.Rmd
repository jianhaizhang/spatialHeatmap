...
To co-visualize single cell with bulk tissue data (Figure
\@ref(fig:covizOver)), the individual cells of the single cell data are mapped
via their group labels to the the corresponding features in an aSVG image. If
...
translation table can be used to avoid manual relabelling. Throughout this
...
container illustrated in Figure \@ref(fig:covizOver).1 [@sce]. The `SCE` slots
`assays`, `colData`, `rowData`, and `reducedDims` contain expression profiles,
...
summarize the expression of a given gene across the cells within a group via a
meaningful summary statistics, such as mean or median. Cells and tissues with
the same group label will then be colored the same. When coloring by bulk data
the color used for each tissue will be applied to the corresponding cell groups
represented in the embedding plot. 
...
To obtain for examples with randomized parameters always the same results, a fixed seed is set for generating random numbers.
...
(`logNormCounts`), and subsequently processed by dimensionality reduction
methods including PCA, TSNE, and UMAP (`reduce_dim`).    
...
are summarized by their source tissues using common summary statistics such as mean 
or median. 
...
The summarized values are then used to color the tissues represented in the
...
Sample features are returned from `feature.df`.

```{r , eval=TRUE, echo=TRUE, warnings=FALSE}
tail(feature.df$feature) 
```

One collapsed cell cycle is equivalent to one aSVG feature. Before
co-visualization, their matching relationship needs to be defined. In
annotation-based and manual methods, the matching relationship is defined by
users in a named `list`, while in the automated method it is computed, which 
is the essential difference across the three methods. Note, one
collapsed cell label can be matched to multiple aSVG features but not vice
versa. The following is an example named `list`.   
...
The co-visualization sample plot is created for gene `Gene_0010`. The embedding
part includes all cells. The `G1` and `G0` cells are colored by their group
expression value summaries (`data=sce.aggr.test`). The source tissues in the
spatial heatmap plot are colored accordingly. The `cell.group` argument
indicates which column is used as cell labels in the `colData` slot of
`sce.aggr.test`.  

```{r , eval=TRUE, echo=TRUE, warnings=FALSE, results='hide', fig.cap=('Example co-visualization plot. The co-visualization is created on gene `Gene_0010`. Single cells in the embedding plot and their matching aSVG features in the spatial heatmap are filled by the same color according to summarized assay profiles within each cell label.')}
...
```
<p style="color:red">Since the example uses summarized expression expression values for 
coloring there needs to be a color key in the plot defining to values the heat colors map to. Without
this the expression information is not shown in the plot. </p>
...
Alternatively, an `SCE` object might already contain this information, such as some of 
the `SCE` objects provided by the `scRNAseq` package [@scrnaseq].    
...
single cell data are pre-processed with common QC, normalization and 
dimensionality reduction steps by the `process_cell_meta` function.
The details of the pre-processing methods are described in the corresponding 
help file. Additional background information on these topics can be found in the [OSCA](http://bioconductor.org/books/3.14/OSCA.workflows/zeisel-mouse-brain-strt-seq.html){target='blank'}
tutorial.
...
A slice of the summarized data is shown below for two genes (Tcea1 and Atp6v1h), where `hypothalamus` and `control` are the grouping and treatment labels, respectively. 
...
feature.df <- return_feature(svg.path=svg.mus.brain)
```
A subset of features is extracted as follows.  

```{r eval=TRUE, echo=TRUE, warnings=FALSE}
tail(feature.df$feature)
```

In practice the labels used for the features in the aSVG images and the the cell groups in the single cell data may not be the same. To resolve this without manual relabeling, a translation list can be used to make them match, where the cell and feature labels are expected to be the names and elements of the list components, respectively. 
...
The following plots the corresponding co-visualization for sample gene Cops5. A co-visualization plot is generated for each experimental variable, here `control` and `6h.post.stress`. The legend under the embedding plots shows the cell labels in the matching list (`lis.match`). The source tissue information is indicated by using matching colors in the embedding and tissue plots on the left and right, respectively. 

```{r scLabVis, eval=TRUE, echo=TRUE, warnings=FALSE, fig.wide=TRUE, fig.cap=('Co-visualizing single cells and bulk tissues by annotation cell labels. The expression profiles of gene `Cops5` are used. Cell population of the same annotation label is colored by their summarized expression profiles under the same condition, and the bulk tissue counterpart is colored by the same color.'), out.width="100%", fig.show='show', results='hide'}
...
```
<p style="color:red">Several things are missing: (1) Similarly as above, a heatcolor key/legend is missing to show/decode experssion information. (2) To track tissue assignment, if expression values are similar across tissues, there needs to be an argument toggle to color one time by expression values and another time by tissue. Otherwise tissue assignment info is lost. (3) I don't understand the plot on the bottom that shows the stress sample. Why does the sp heatmap highlight in this case only one tissue? </p>
...
```{r eval=TRUE, echo=TRUE, warnings=FALSE, fig.wide=TRUE, fig.cap=('Embedding plot of single cells. The cells (dots) are colored by the grouping information by the `colData` slot of the corresponding `SCE` object .'), out.width="100%", fig.show='show'}
...
The automated method (Figure \@ref(fig:covizOver).2c) assigns source bulk
tissues to individual cells in an automated manner through a co-clustering
algorithm (Figure \@ref(fig:coclusOver)). This section showcases this algorithm
and its application on co-visualization.   

### Algorithm Overview


The most desirable input bulk and single cell data would be raw count data (e.g. RNA-seq) derived from the same organ or
tissue (Figure \@ref(fig:coclusOver).1). If same organ, the chosen bulk tissues are major or complete tissues comprising this organ, while the single cell data are assayed on the region covering at least the chosen bulk tissues or on the whole organ. Similarly, if chosen bulk tissues are sub-tissues composing partial or the complete tissue, the single cells need to cover at least the chosen sub-tissues or the whole tissue. The key point is bulk tissues and single cells have anatomical overlap. In the pre-processing step (Figure \@ref(fig:coclusOver).2), the count data of bulk tissues and single cells are initially filtered respectively to remove the noisy data for the subsequent normalization, which is explained in the downstream [Pre-processing](#proCoclus) step. The main difference between
bulk and single cell data is the sparsity in the latter, where rows are molecules (genes, *etc*) and columns are bulk tissues or cells respectively. To reduce such difference, they are combined in a column-wise manner and normalized as a whole, then subsequently separated (Figure \@ref(fig:coclusOver).2b-c).     
