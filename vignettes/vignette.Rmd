...
connections among nodes. Third, the adjacency matrix is used to calculate a
topological overlap matrix (TOM) where shared neighborhood information among
nodes is used to preserve robust connections, while removing spurious
...
`data.frame`. Since the [interactive network](#inter_net) feature used below performs 
best on smaller modules, only modules are returned that were obtained with stringent `ds`
