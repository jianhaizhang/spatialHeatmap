...
with `spatialHeatmap`, slight modifications had to be introduced in their SVG files. 
The fully annotated and ready-to-use aSVG image collection of the Expression Atlas is 
...
functionalities of the `spatialHeatmap` package using as first example a simple
...
The `returnSVGFeatures` function can be used to return the feature names present 
in an aSVG image.
...
spatial_hm(svg.path=svg.hum, data=my_vec, col.com=c("yellow", "blue", "purple"))
...
plotted onto a chosen aSVG image. In this case, the query and download
...
Additional details for generating and using targets files in `spatialHeatmap`
...
result also doesn't seem to be log ratios. You need to better describe what you
...
kable(assay(se.fil.hum)[733:735, ])
``` 

The following shows how to download the corresponding pre-annotated aSVG image from 
the [SVG repository](#svg_tmp) of this package. 
...
As explained in the previous section, the same aSVG image has been included in this 
package. To meet the requirements for building vignettes in R packages, the following 
code section uses the packaged instance of the aSVG file rather than downloading it.
...
spatial_hm(svg.path=svg.hum, data=se.fil.hum, ID=c('ENSG00000268433'), col.com=c("yellow", "blue", "purple"), width=1, height=0.4, legend.r=0.5, sub.title.size=11, layout="gene", ncol=2, tis.trans=NULL, sam.legend='identical', line.size=0.2, line.color='grey70', legend.position=c(0.5, -0.15), legend.nrow=1)
...
1](#brain_shm)) with the legend plot on the right. For example, in [Table
...
ALS and could contribute to hypothesis generation. Note that only frontal
cortex and cerebellum are colored while others are blank in the spatial
heatmaps, since they are the only 2 identical tissues between the data matrix
and SVG image.  
...
# Normalise.
se.nor.mus <- norm_data(data=rse.mus, norm.fun='CNF', data.trans='log2')
# Aggregate replicates.
...
spatial_hm(svg.path=svg.mus, data=se.fil.mus, ID='ENSMUSG00000000263', col.com=c("yellow", "blue", "purple"), width=1, height=0.5, legend.r=0.6, sub.title.size=7, layout="gene", ncol=3, tis.trans='skeletal.muscle', sam.legend='identical', legend.key.size=0.3, legend.nrow=4, legend.position=c(0.5, -0.15))
...
# Normalise.
se.nor.chk <- norm_data(data=rse.chk, norm.fun='CNF', data.trans='log2')
# Aggregate replicates. 
...
spatial_hm(svg.path=svg.chk, data=se.fil.chk, ID='ENSGALG00000006346', col.com=c("yellow", "blue", "purple"), bar.width=0.7, width=1, height=0.9, legend.r=0.4, sub.title.size=7, layout="gene", ncol=3, legend.key.size=0.4, legend.label.size=7.5, legend.position=c(0.5, -0.15), legend.nrow=2)
...
spatial_hm(svg.path=svg.sh, data=se.fil.sh, ID="HRE2", col.com=c('purple', 'cyan', "brown"), width=1, height=0.5, layout="gene", ncol=2, legend.nrow=3, legend.r=0.8, legend.key.size=0.3, legend.label.size=7, legend.position=c(0.5, -0.07))
...
spatial_hm(svg.path=svg.hum, data=se.fil.hum, ID='ENSG00000268433', col.com=c("yellow", "blue", "purple"), width=1, height=0.5, sub.title.size=11, layout="gene", ncol=2, tis.trans=NULL, sam.legend='identical', line.size=0.2, line.color='grey70', legend.position=c(0.5, -0.15), legend.nrow=1)
