...
<font color='blue'>jianhai: I will discuss with you on slack on this topic.</font> ^[Naming the subdirectory 'UCR' where you store the images in the repos 
is a poor choice as it indicates a personal rather than a community solution. Perhaps name it just 
'svg_images'? Also try to include there more images. The current collection is so 
small that it is of questionable value.]
...
```{r arabshm, eval=TRUE, echo=TRUE, warnings=FALSE, fig.wide=TRUE, fig.cap=('SHMs of Arabidopsis at two growth stages. The expression profile of gene1 under condition1 and condition2 is plotted for two growth stages (top and bottom row).'), out.width="100%"}
...
^[<font color='red'>It seems submatrix creates internally an all-against-all similarity/distance
matrix instead of performing a similarity search and returning only the
relevant items. The computation of the full similarity matrix is a quadratic problem
that will crash user's systems when the assay data contains many genes by
taking up all RAM, whereas a search is of linear complexity, and thus scales to much larger 
assay data sets. The run-time in R for a correlation search is very fast since 
there is no R-based looping necessary (simply do: cor(query_vec, matrix)) as it 
runs on the C/Fortran level, meaning speed is not a concern here.</font>] ^[<font color='blue'>Now the similarity/distance matrix is only computed on relevant genes.</font>]
...
with the query genes 'RCA' and 'HRE2'.  ^[<font color='red'> Again why would you compute the full
similarity matrix for all genes in the assay and then even return it here. If
you need to return it, then compute it only for the filtered results meaning it
should have dim= 62 x 62 and not 319 x 319.</font>] ^[<font color='blue'>The returned value is changed to only the submatted matrix.</font>]
...
<font color='blue'>jianhai: It works on my tmux session, where `getOption("browser")` retruns "xdg-open". First, try to run `library(plotly); subplot(plot_ly(economics, x = ~date, y = ~unemploy))`. If nothing shown, it suggests the issue is related to RStudio settings. Second, a universal solution is `mhm <- matrix_hm(...);` `htmlwidgets::saveWidget(widget=mhm, file='mhm.html', selfcontained=FALSE);` `browseURL('mhm.html')`, where "..." denotes the passed arguments. This solution saves the interactive heatmap to "mhm.html". I have added this solution to the example section of the help in case some users also have this issue. </font> ^[<font color='red'>ThG: the interactive mode doesn't work for me. Am I supposed to get a URL like under
the network plot? Right now it opens RStudio but doesn't show there anything.</font>]
...
<a name="ds"></a>
Network analysis is performed with the WGCNA algorithm [@Langfelder2008-sg; @Ravasz2002-db] 
using as input the subsetted assay matrix generated in the previous section. The objective 
is to identify network modules that can be visualized in the following step in form of network graphs.^[<font color='blue'>Should the objective emphasize explore the target item in context of NNs rather than only network visualization? To achieve such exploration, the matrix heatmap using hierarchical clustering is basic/traditional and WGCNA using TOM is advanced. </font>] Applied to the 
gene expression sample data used here, these network modules represent groups of genes 
sharing highly similar expression profiles. Internally, the network module identification
includes five major steps. First, a correlation matrix is computed using 
distance or correlation-based similarity metrics. Second,
the obtained matrix is transformed into an adjacency matrix defining the
...
performed with the `flashClust` package [@flashClust]. Fifth, network modules
are identified with the `dynamicTreeCut` package [@dynamicTreeCut]. Its
`ds` (`deepSplit`) argument can be assigned integer values from `0` to `3`, where higher 
values increase the stringency of the module identification process. 
To simplify the network module idenification process with WGCNA, the individual steps 
can be executed with a single function called `adj_mod`. The result is a list 
containing the adjacency matrix and the final module assignments stored in a 
`data.frame`. Since the [interactive network](#inter_net) feature used in the visualization
step below performs best on smaller modules, only modules are returned that were obtained with stringent `ds`
settings (here `ds=2` and `ds=3`). 
^[<font color='red'>The help file for adj_mod contains many typos and grammar mistakes. Please
improve its writeup.</font>]

^[<font color='blue'>I have improved the adj_mod help file as well as all other help files. Please let know if further improvement is needed.</font>]
...
Network modules can be visualized with the `network` function. To plot a module containing
an item (gene) of interest, its ID (_e.g._ 'HRE2') needs to be provided under the corresponding argument. 
Typically, this could be one of the items chosen for the above SHM plots. There are two 
modes to visualize the selected module: static or interactive. Figure \@ref(fig:inter)
was generated with `static=TRUE`. Setting `static=FALSE` will generate the interactive
version. In the network plot shown below the nodes and edges represent genes and adjacencies 
between them, respectively. The thickness of the edges denotes the adjacency levels, while the size of 
the nodes indicates the degree of connectivity of each item. <font color='blue'> In addition, the adjacency and connectivity levels are also indicated by colors. For example, in Figure \@ref(fig:inter) connectivity increases from "turquoise" to "violet" while adjacency increases from "yellow" to "blue". </font> ^[<font color='red'>ThG: the color scheme of the nodes is a poor choice since the labels are unreadable in the dark brown 
nodes. Also the color scheme is not explained.</font>] If a gene of interest has 
been assigned under `ID`, then its ID is labeled in the plot with the tag: '*_target'.
...
If the subsetted expression matrix contains gene/protein annotation information in the last
column, then it will be shown when moving the cursor over a network node.    

```{r eval=FALSE, echo=TRUE, warnings=FALSE}
network(ID="HRE2", data=sub.mat,  adj.mod=adj.mod, static=FALSE)
```
<font color='blue'>jianhai: It was set empty since too many edges could lead to browser crash. Now the solution is upon launched all nodes are shown and no edges are shown (adjacency threshold is set 1 by default). Users could decrease the threshold to shown edges. The instruction page is added to tell how to operate in details. </font>
^[<font color='red'>ThG: the interactive view shows only an empty shiny like window. Even when trying to change
the settings in the menu on the left nothing happens.</font>]

# Shiny App {#shiny}

 <font color='blue'>In additon to running `spatialHeatmap` in command mode from R, the package includes a GUI through the [Shiny
App](https://shiny.rstudio.com/) that provides an intuitive-to-use web browser interface. </font> <font color='blue'>jianhai: duplicate text was removed. </font> ^[ThG: the text you inserted in this paragraph is covered already in the R
section. If we state that the shiny app provides the same functionalities then
this doesn't need to be mentioned again.]
...
The assay data and aSVG images are uploaded to the Shiny App
as tabular (_e.g._ in CSV or TSV format) and SVG files, respectively. To
...
desired directory path under `dir`. <font color='blue'>Within this directory the tabular file will 
be written to `customData.txt` in TSV format </font>. The
...
<font color='blue'>jianhai: to show the network, Select a target gene, the adjacency threshold, and "Yes" under "Show plot" should be seletcted. The most intensive computation lies in the TOM of WGCNA, which is done on the subsetted matrix. If the subsetted matrix has 10,000+ rows, the system very likely crashes during the TOM computation. So it can probably happen if users provide large matrix. This is the first reason why network is not shown by default. The second reason is most of the time users are likely to use the SHM only and do not care the matrix heatmap or network. If network and matrix heatmap are updated in real time with SHMs, the overall speed would be reduced obviousely. Users could click as many as 3 genes per second, each click will trigger SHM update. If they submit 10+ genes in the search box, SHMs are also updated. If these updates are synced to matrix heatmap and network, the app is likely running slowly. The current strategy is remove matrix heatmap and network every time SHMs/input genes are updated, which is also applied to animation and video. If users want to see them then check "Yes" under "Show plot". This design ensures optimal performance on the core featue of SHM. Does it makes sense? </font>
<font color='red'>
^[I wasn't able to get the network panel in the shiny app to work. Only the 
SHM and matrix heatmap panels returned a result. I am also wondering why showing all
three upon app load (SHM, matrix heatmap and network graph) isn't the default. With the
recent speed optimizations this shouldn't be a problem anymore.]
</font>
...
<font color='blue'>In either local or server application, if the subsetted matrix contains too many rows such as more than 10,000 in local mode, the app might collapse, which depends on the system RAM. The solution is to increase subsetting strigency in the Matrix Heatmap panel so as to reduce the subsetted matrix, which would avoid system crash most of the time. In rare cases users have to use very large submatted matrix, the "customComputedData" under "Step1: data sets" is suggested. Refer to function `adj_mode` and `submatrix` for more details. Note, if only the SHM panle is used, there is no such potential issue. </font>

<font color='blue'>jianhai: I have made a note of the SQLite or HDF5 database development.</font>
<font color='red'>
^[What you wrote here should be resolved now with the recent updates focusing the hierarchical
clustering and network analysis to the NNs of a query gene(s). What you should address at some 
point, however, is that the expression data can be read from an SQLite or HDF5 database to 
support more complex data sets (e.g. all tissue specific Arabidopsis data sets in GEO) on the 
server (and client) side of web deployments.]
</font> 

## Custom Shiny App
<font color='blue'>

The `spatialHeatmap` package also enables users to create customized Shiny apps
through the function `custom_shiny`. Specifically, users have options to
include their own example data and aSVGs, set their preferred default values
(*e.g.* color scheme, image dimension). See the help file of `custom_shiny` for
details. ^[<font color='blue'>I didn't include example code snippet of using
this functionality, since this vignette is already long enough. Is it
reasonable? </font>] In addition, on the Shiny app a template of default
parameters in yaml format is provided for download. Users can change the
parameters in this file and upload it back to the app, then the default
...
Group](https://github.com/ebi-gene-expression-group/anatomogram/tree/master/src/svg).
...
a step-by-step [SVG tutorial](https://jianhaizhang.github.io/SVG_tutorial_file/) for creating
custom aSVG images. For example, the [BAR eFP browser](http://bar.utoronto.ca/) at University 
