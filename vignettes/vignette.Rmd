...
In the gene expression matrix, the row and column names should be gene IDs and sample/conditions respectively. The sample/condition names MUST be fomatted this way: a sample name is followed by double underscore then the condition. For example, in the column name "epidermis__140mM_1h" in [Table 1](#table), "epidermis" is the sample and "140mM_1h" is the condition. Additionally, the column names of sample/condition can only consist of letters, digits, single underscore, single space, or dots. Not all samples in the matrix need to be present in the [SVG image](#svg_form), vice versa. Only samples common between the data matrix and the SVG image are recognised and coloured.
...
svg.path <- system.file("extdata/example", "homo_sapiens.brain.svg", package = "spatialHeatmap")
spatial.hm(svg=svg.path, data=exp.ebi, pOA=c(0, 0), CV=c(0, 10000), ID=rownames(log.df)[6], col.com=c("yellow", "blue", "purple"), width=1, height=0.5, sub.title.size=7, layout="gene", ncol=3, tis.trans='cerebral_cortex')
