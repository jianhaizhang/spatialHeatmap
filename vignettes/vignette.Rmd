...
`SummarizedExperiment` object. <font color='blue'> In cases of simple assays, there is no need to use the advanced `SummarizedExperiment`, and the data can come as a `vector` or `data frame`. The former applies to several numeric values measured for a single item (gene, protein, metabolite, *etc*), while the latter applies to more items assayed in several samples and/or condidions, *e.g.* two tissues under two conditions. Details about how to access the SVG images and properly format the associated expression data are provided in the [Supplement](#data_form) section of this vignette. </font>
...
dir.create('~/test')
# Query aSVGs.
feature.df <- return_feature(feature=c('frontal cortex', 'cerebellum'), species=c('homo sapiens', 'brain'), keywords.all=FALSE, return.all=FALSE, dir='~/test', remote=TRUE, match.only=TRUE, desc=FALSE)
# First 8 rows.
feature.df[1:8, ]
# All matching aSVGs.
...
```{r eval=TRUE, echo=TRUE, warnings=FALSE }
feature.df <- return_feature(feature=c('frontal cortex', 'cerebellum'), species=c('homo sapiens', 'brain'), keywords.all=FALSE, return.all=FALSE, dir=svg.dir, remote=FALSE)
```
...
jianhai: `DataFrame` should be used otherewise errors arise.  
...
mapping, this tissue is coloured blue corresponding to 5.324064 in the color
...
<font color='blue'> 
Download aSVGs from remote EBI repository directly based on the experiment design. An empty directory '~/test' is suggested to save the downloaded files so as to avoid overwriting existing files.
...
<font color='blue'> 
Download aSVGs from EBI repository based on the experiment design. An empty directory '~/test' is suggested to save the downloaded files so as to avoid overwriting existing files.  
...
```{r eval=TRUE, echo=TRUE, warnings=FALSE }
feature.df <- return_feature(feature=c('pGL2', 'pRBCS'), species=c('shoot'), keywords.all=FALSE, return.all=FALSE, dir=svg.dir, remote=FALSE, match.only=FALSE)
```
...
Make a vector of new feature identifiers corresponding to every returned feature, *e.g.* replacing space with dots. This vector must be added to the first column of the feature data frame, since that is where `update_feature` looks for new features. Then features are updated with `update_feature`.    
