...
our guidlines.^[Naming the subdirectory 'UCR' where you store the images in the repos 
...
```{r eval=TRUE, echo=TRUE, warnings=FALSE}
sub.mat[['sub_matrix']][c('RCA', 'HRE2'), c(1:3, 37)] # Subsetted assay matrix
```
...
^[<font color='red'>ThG: the interactive mode doesn't work for me. Am I supposed to get a URL like under
...
is to identify network modules that can be visualized in the following step in form of network graphs. Applied to the 
...
the nodes indicates the degree of connectivity of each item. If a gene of interest has 
been assigned under `ID`, then its ID is labeled in the plot with the tag: '*_target'.
^[<font color='red'>ThG: the color scheme of the nodes is a poor choice since the labels are unreadable in the dark brown 
nodes. Also the color scheme is not explained.</font>]  
...
^[ThG: the text you inserted in this paragraph is covered already in the R
...
`spatialHeatmap` includes a [Shiny App](https://shiny.rstudio.com/){target="_blank"} that
...
>>>>>>> 314f1d2a2115a63edaa248e6f43f8e11c5e18e72
...
=======
<font color='blue'>
Lastly, in either local or online deployment there is a potential computation issue if the subsetted matrix contains too many rows such as more than 10,000. Specifically, too many rows would cause intensive computation in the network analysis and potentially lead to system crash. To prevent this potential issue, the Shiny app is added an option of "customComputedData". So users can get the intensive network analysis computed outside Shiny and then upload the computed results. In this case three files should be uploaded: filtered data matrix, adjacency matrix, module assignment, where the last two are specifically generated on the first one. The details are documented in help files of function `filter_data` and `adj_mod`.  
...
The `spatialHeatmap` package also enables users to create customized Shiny apps through the function `custom_shiny`. Specifically, users have options to include their own example data and aSVGs, set their preferred default values (*e.g.* color scheme, image dimension). See the help file of `custom_shiny` for details. ^[<font color='blue'>I didn't include example code snippet of using this functionality, since this vignette is already long enough. Is it reasonable? </font>] In addition, on the Shiny app a template of default parameters in yaml format is provided for download. Users can change the parameters in this file and upload it back to the app, then the default settings are changed accordingly.  
...
generated by the [EBI Gene Expression Group](https://github.com/ebi-gene-expression-group/anatomogram/tree/master/src/svg){target="_blank"}. It
...
repository called [spatialHeatmap aSVG Repository](https://github.com/jianhaizhang/spatialHeatmap_aSVG_Repository){target="_blank"}, where some aSVG files developed in this project are already deposited (*e.g.* Figure \@ref(fig:shshm)).  ^[It
...
tutorial](https://jianhaizhang.github.io/SVG_tutorial_file/){target="_blank"} for creating
custom aSVG images. For example, the [BAR eFP browser](http://bar.utoronto.ca/){target="_blank"} at University of Toronto contains many anatomical images, and these images can be used as templates in the SVG tutorial to make custom aSVGs.  
>>>>>>> 314f1d2a2115a63edaa248e6f43f8e11c5e18e72
