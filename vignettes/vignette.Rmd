...
heatmap are usually quantitative measurements from a wide range of profiling
...
biomedical areas. However, the package has been designed to also work with many
other spatial data types, such a population data plotted onto geographic maps.
This high level of flexibility is one of the unique features of _spatialHeatmap_.
Existing software in this field is largely based on pure web applications
[@Winter2007-bq; @Waese2017-fx] or local tools [@Maag2018-gi;
@Muschelli2014-av] that typically lack customization functionalities by
restricting users to utilizing pre-existing expression data and/or fixed sets
of anatomical image collections. To close this gap for biological use cases, 
we have developed _spatialHeatmap_ as a generic R/Bioconductor package
for plotting quantitative values onto any type of spatially mapped images in a
programmable environment and/or in an intuitive to use GUI application.

## Design

The core feature [Spatial Heatmap](#shm) is to map the assay values (_e.g._
...
assay data along with the associated experimental design information in a
...
functionalities of the `spatialHeatmap` package using mRNA profiling data as
an example. First, spatial heatmap plots are generated for four gene
expression data sets including both RNA-seq and microarray experiments from
...
## Terminology
SVG is a widely used vector graphics format where XML formatted text describes
all lines, shapes and text in images, such as drawings of anatomical
structures. To assign colors to specific features in spatial heatmaps,
"annotated SVG" files have to be used where the shapes of interest are labeled
according to certain conventions so that they can be colored programmatically.
Annotated SVGs of anatomical structures can be downloaded from the repositories
mentioned above, or users can generate them themselves. Typically, in annotated
SVGs one or more shapes making up a feature of interest, such as the cell
shapes of an organ, are grouped together by a common feature identifier in the
SVG. Via these identifiers one or many feature types can be colored
simultaneously in an SVG according to biological experiments assaying the
corresponding feature types with the required spatial resolution. Correct
assignment of image features and assay results is assured by using for both the
same feature identifiers. The color gradient used to visually represent the
numeric assay values is controlled by a color gradient parameter. For
convenience the corresponding color key can be included in the spatial heatmap
plots. Additional formatting details of both SVG images and assay data are
provided in [Supplement Section](#sup) of this vignette. 
...
Spatial heatmaps are plotted with the `spatial_hm` function. To illustrate how
these plots are generated, the following uses a generalized toy example where a
vector of random numeric values is generated that are used to color the
corresponding features in a pre-annotated SVG image. The image chosen for this
example is from the human brain. To map the numbers to the correct features in
the image they are paired by a name matching approach between the name slot of
the numeric vector and features names of the image. To obtain the feature names
present in the annotated SVG, the `returnSVGFeatures()` function can be used.
The obtained names of the image are stored in a character vector and then
assigned to the name slot in the numeric vector provided by the user. Next, the
numbers are translated to colors based on a user definable color key and then
painted onto the corresponding named features in the image.

<font color="red">
[ThG-Comment 3: Jianhai, please add a code section that accomplishes what is described
in the above text. This will help to give the user a quick code-based intro into the
...
tissues in the legend, and the default "identical" only shows the coloured
tissues.   
...
adjust the thickness and colour of polygon outlines respectively and thus can
