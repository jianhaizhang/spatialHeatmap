...
repository called [spatialHeatmap aSVG Repository](https://github.com/jianhaizhang/SVG_tutorial_file/tree/master/GirkeLab_SVG_Repo), 
where users can contribute their aSVG images that are formatted according to our guidlines.
^[GirkeLab in the name of this repos is a poor choice. Perhaps just call it spatialHeatmap_aSVGs.]
<font color='blue'>jianai_comment: SVG repo name changed</font>
...
^[The numeric values range from 9-26, while the color key on the 
left has a range from 0-2500. This is confusing and should be corrected.]
<font color='blue'>jianai_comment: This issue stems from the new ggolot2 pacakge. I reported it to ggplot2 github and they said it will be fixed in next version. For the current release of ggplot2, I added a solution in spatialHeatamp. Re-rendering the vignette without using old cache  will restore the scale. </font>
...
<font color='blue'>UBERON ids only applied to animals, but the EBI SVG also includes plant ontology ids. So is "https://www.ebi.ac.uk/ols/index" a better choice for the link?  </font>
...
preprocessing steps are optional, and thus can be skipped as needed. For instanse, fully preprocessed data can be applied to the function `spatial_hm` for [plotting spatial heatmaps](#hum_single) directly by skipping the preprocessing routines, but the sample column name and/or condition column name in the `colData` slot must be provided to `sam.factor` and/or `con.factor` respectively (Table \@ref(tab:arg)).  
</font>
...
<font color='blue'>jianhai_reply: Can be corrected by re-rendering without using old cache.</font>
...
The following selects `mus_musculus.male.svg` as the target aSVG, returns the first
three rows of the resulting `feature.df` and also prints the unique set of all
aSVG features.   
...
To instect its content, the first three rows of the target file are printed to the 
screen.
...
The raw RNA-Seq count are preprocessed with the following steps including (1)
...
shape outlines. The `line.size` and `line.color` parameters are used to
adjust the thickness and color of shape outlines respectively and thus enhance the visualization. In Figure \@ref(fig:musshm1), the default values of the 2 arguments are used.   
</font>
