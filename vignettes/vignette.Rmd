...
and cell-specific gene expression data in form of colors plotted onto the
corresponding spatial features mapped in anatomical images. The color scheme
used to represent the gene expression values can be customized by the user.
This core functionality is called spatial heatmap. It is enhanced with gene
network visualization tools, including matrix heatmaps combined with
hierarchical clustering dendrograms and network representations of groups of
genes (_e.g._ gene modules) sharing related expression patterns. The
...
convenience features that will appeal to many biologists. Moreover, the Shiny
App has been designed to work on both local computers as well as server-based
deployments (_e.g._ cloud-based or custom server) that can be access remotely 
as a centralized community web service.
...
microarray experiments from [Human Brain](#hum), [Mouse Organs](#mus), [Chicken Organs](#chk), and
[Arabidopsis Shoots](#shoot). Second, gene context visualization features
