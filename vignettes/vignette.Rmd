...
can refer to in the text with Figure 1A or Figure 1B, etc.]</font>
...
field are largely based on pure web applications ^[<font color='blue'> Semantic Body Browser [@Lekschas2015-gd], and the Expression Atlas (https://www.ebi.ac.uk/gxa/home) both are able to visualize data on anatomical images and are web-based, should they be cited? </font>][@Winter2007-bq;
...
example (Figure \@ref(fig:musshm1)). <font color='blue'> The SHM functionality also accepts multiple aSVGs as shown in Figure \@ref(fig:arabshm). This is particularly useful when map data to tissues across multiple development phases. In addition to static representation, SHMs are also presented in an anmation in the Shiny App. This functionality displays SHMs sequentially according to genes or conditions in the same window, and allows zoom in and out at specific shapes.</font>      
...
metabolic profiling data from databases, such as [GEO](#https://www.ncbi.nlm.nih.gov/gds), [BAR](#http://bar.utoronto.ca/) and Expression Atlas from EMBL-EBI [@Papatheodorou2018-jy] ^[<font color='blue'>jianhai: added link.</font>]. A
particularly useful resource, when working with `spatialHeatmap`, is the EBI Expression
Atlas. This online service contains both assay and
...
repository called [spatialHeatmap aSVG Repository](https://github.com/jianhaizhang/spatialHeatmap_aSVG_Repository), 
...
The following example shows how to download from the <font color='blue'> previously mentioned [SVG
repositories](#data_repo) </font> an aSVG image that matches the tissues and species
...
feature and one species match, ^[<font color='blue'>jianhai-comment: at least one feature cannot be guaranteed, since the SVG repo is not huge. "To return all possible matching aSVGs" is more accurate.</font>] the argument `keywords.any` is set to `TRUE` by
...
```{r mul, eval=TRUE, echo=TRUE, warnings=FALSE, fig.wide=TRUE, fig.cap=("SHMs of two genes. The subplots are organized by \"condition\" through `lay.shm` argument."), out.width="100%" }
...
Plot SHMs on `gene1`. The second aSVG is used for legend plot through `legend='shm2'` and orginal dimension relativity is preserved by `preserve.scale=TRUE`.    
...
spatial_hm(svg.path=c(svg.sh1, svg.sh2), data=df.random, ID=c('gene1'), legend.r=1.6, preserve.scale=TRUE, legend='shm2') 
```
...
simply by clicking the corresponding rows in the expression table included in the same window. <font color='blue'> The subplots of SHMs, including multiple growth stages, are presented in two modes "Basic" and "Animation". The former compiles all subplots in the same page, and the dimension, layout, and color key are all customizable, while the latter displays each subplot sequentially in an animation. The animation can be played continuously or paused at a frame. Each frame can be zoomed in and out by drawing a rectangle and clicking the "home" icon in the top-right toolbar respectively. When mouse over a shape, the information of gene, condition, feature identifier, and value are displayed in a tootip. </font> This representation is very efficient in guiding the interpretation of the results 
...
<font color='blue'>jianhai-reply: Missing pre-uploaded files are added, and now it is functional. The matrix heatmap and network section in the app are not synced with the vignette, since the vignette is not final yet.</font>
