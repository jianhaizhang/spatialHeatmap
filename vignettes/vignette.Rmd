...
Spatial heatmaps are images where colors encode numeric values. For plotting
spatial heatmaps, the Scalable Vector Graphics (SVG) format has been chosen as
image encoding since it is a flexible and widely adapted vector graphics format
that provides many advantages for computationally embedding numerical
information in images. SVG is based on XML formatted text describing all
lines, shapes and text in images.  In case of biological images the shapes
often represent anatomical or cell structures. To assign colors to specific
features in spatial heatmaps, _annotated SVG_ (aSVG) files have to be used
where the shapes of interest are labeled according to certain conventions so
that they can be colored programmatically. Raw SVGs and aSVGs of anatomical
