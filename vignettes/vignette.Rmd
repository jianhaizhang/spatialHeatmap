...
example (Figure \@ref(fig:musshm1)). <font color='blue'> In addition to static representation, SHMs are also presented in an anmation in the Shiny App. This functionality displays SHMs sequentially according to genes or conditions in the same window, and allows zoom in and out at specific shapes.</font>      
...
simply by clicking the corresponding rows in the expression table included in the same window. <font color='blue'> The subplots of SHMs are presented in two modes "Basic" and "Animation". The former compiles all subplots in the same page, and the dimension, layout, and color key are all customizable, while the latter displays each subplot sequentially in an animation. The animation can be played continuously or paused at a frame. Each frame can be zoomed in and out by drawing a rectangle and clicking the "home" icon in the top-right toolbar respectively. When mouse over a shape, the information of gene, condition, shape identifier, and value are displayed in a tootip. </font> This representation is very efficient in guiding the interpretation of the results 
