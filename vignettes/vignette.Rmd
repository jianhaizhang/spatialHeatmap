...
using as input the subsetted assay matrix generated in the previous section
(here gene expression matrix). The objective of this network analysis step is
the identification of network modules. In the context of the sample data used
here, the network modules represent groups of genes sharing highly similar
expression profiles that can be visualized in form of correlation-based network
graphs. First, a correlation matrix or distance matrix is computed on the
subsetted matrix using Pearson correlation coefficients or Euclidean distances,
respectively. Subsequently, the obtained matrix is transformed to an adjacency
matrix. A characteristic feature of adjacency matrix-based networks is a sale-free
topology, where only a small number of items have many connections with other
items. Thus, a low degree of interconnectivity is a dominating feature of the
overall network [@Ravasz2002-db]. The values in the adjacency matrix denote
profile similarities among genes, where larger values indicate higher
similarity.

Next, the adjacency matrix is used to calculate a topological overlap matrix
(TOM). This advanced similarity measure quantifies expression similarities for
each pair of genes in the context of all other genes. Next, the distance
transformed TOM matrix ($1-TOM$) is used for hierarchical clustering with
`flashClust` [@flashClust]. Finally, network modules are identified with the
`dynamicTreeCut` package [@dynamicTreeCut]. It provides an argument `deepSplit`
(`ds`) for rough control over sensitivity to module splitting. The `ds` has
four alternative values `0`, `1`, `2`, `3`, and each generates an alternative
set of modules.  The higher the value, the more and smaller modules are
produced, thus there are four alternative sets of modules generated with
varying sizes.

The whole process above of identifying modules is implemented in the function
`adj_mod`. It returns a list containing the adjacency matrix and a data frame
of module assignment. Since the [interactive network](#inter_net) below
performs better on smaller modules, only modules resulting from `ds=2` and
`ds=3` are returned.  
