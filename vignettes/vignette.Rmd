...
runs on the C/Fortran level, meaning speed is not a concern here.</font>]
...
The result from the previous step is stored in a list where the first component
contains the assay matrix subsetted to the genes sharing similar assay profiles
with the query genes 'RCA' and 'HRE2'. The second slot contains the corresponding
PCC similarity matrix. ^[<font color='red'> Again why would you compute the full
...
should have dim= 62 x 62 and not 319 x 319.</font>]

```{r eval=TRUE, echo=TRUE, warnings=FALSE}
sub.mat[['sub_matrix']][c('RCA', 'HRE2'), c(1:3, 37)] # Subsetted assay matrix
sub.mat[['cor_dist']][1:3, 1:3] # PCC similarity matrix
```
...
selecting subsections in the image or double clicking on areas of interest.
