...
_annotated SVG_ (aSVG) files have to be used where the shapes of interest are
labeled according to certain conventions so that they can be colored
programmatically.  aSVGs of anatomical structures can be downloaded from the
repositories mentioned above, or users can generate them themselves. Typically,
in aSVGs one or more shapes making up a feature of interest, such as the cell
shapes of an organ, are grouped together by a common feature identifier in the
aSVG. Via these identifiers one or many feature types can be colored
simultaneously in an aSVG according to biological experiments assaying the
...
plots. Additional formatting details of both aSVG images and assay data are
