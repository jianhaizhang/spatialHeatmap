...
can be colored programmatically.  SVGs and aSVGs of anatomical structures can
...
corresponding color key can be included in the spatial heatmap plots.
...
particularly useful resource to combine with `spatialHeatmap` is the Expression
Atlas from EMBL-EBI [@Papatheodorou2018-jy], which contains both assay and
...
provided to work with the expression and images from the Expression Atlas
directly. To make the SVG images from the Expression Atlas fully compatible
with `spatialHeatmap`, slight modifications had to be introduced in them. The fully 
annotated and ready-to-use aSVG image collection of the Expression Atlas is 
available in the [SVG repository](#svg_tmp) of this package. Apart from posting 
future image updates from the Expression Atlas and other public sources, the 
same SVG repository is used to deposit custom images developed by this project 
and/or those contributed by users.
...
understand toy data set as well as mRNA profiling data from the Expression Atlas. 
First, spatial heatmap plots are generated for both the toy and mRNA expression data. The latter
...
[Arabidopsis Shoots](#shoot). The first three are RNA-Seq data from the 
[Expression Atlas](https://www.ebi.ac.uk/gxa/home) and the last one is a
microarray data set from [GEO](https://www.ncbi.nlm.nih.gov/geo/).  Second,
gene context visualization features are introduced, which facilitate the
visualization of gene modules sharing similar expression patterns. This
includes the visualization of hierarchical clustering results with traditional
matrix heatmaps ([Matrix Heatmap](#mhm)) as well co-expression network plots
...
chosen for this example is from the human brain. The latter has been included
in this package for testing purposes. The path to the file of this image on a
user's system, where `spatialHeatmap` is installed, can be obtained with the 
...
A small numeric toy vector is generated, where the data slot contains five
numbers and its name slot is populated with the feature names obtained from the
aSVG image. Note, the numbers are mapped to features via names matching among the 
...
name matches are indicated by a message printed to the R console. This behavior can 
be turned off with `verbose=FALSE` in the corresponding function call. In addition, 
a summary of the numeric assay to feature mappings is stored in the result object 
...
This subsection demonstrates how to find cell- and tissue-specific assay data in the 
Expression Atlas. After choosing a gene expression experiment, the data is downloaded
directly into a user's R session and normalized. Subsequently, the processed expression
values of user selected genes are plotted onto a chosen aSVG image. In this case, the 
query and download functionalities of expression data are provided by functionalities 
of the `ExpressionAtlas` package. 

The following example searches for expression samples from 'cerebellum' from 'Homo sapiens'.
...
from Prudencio _et al._ [-@Prudencio2015-wd] is chosen below which corresponds
to an RNA-Seq profiling experiment of cerebellum and frontal cortex brain
tissue from patients with amyotrophic lateral sclerosis (ALS). The
corresponding record looks as follows.
...
`rse.hum`. The following returns only its first four rows and first five columns.

```{r eval=TRUE, echo=TRUE, warnings=FALSE }
colData(rse.hum)[1:4, 1:5]
```

To shorten the text in some of the columns of `colData`, a simplified version
of this information is provided by this package. In the following example this so called
'targets' file is imported and used to replace the longer version in the `colData` slot. 
It is important to note that for this well annotated data set from the Expression Atlas
this step is optional. It mainly serves the purpose of shorting the text in some 
columns to automatically assign downstream 'pretty' labels to the headers and legends of tables and plots, respectively,
in a controlled manner. If this utility is needed by users, the details for generating 
the corresponding targets files are provided in the [Supplement](#sup) of this
vignette.


```{r eval=TRUE, echo=TRUE, warnings=FALSE }
hum.pa <- system.file('extdata/shinyApp/example/target_human.txt', package='spatialHeatmap')
target.hum <- read.table(hum.pa, header=TRUE, row.names=1, sep='\t')
colData(rse.hum) <- DataFrame(target.hum)
```

A slice of the simplified `colData` object is shown below, where the `disease` column
contains now shorter labels than before.
...
often beneficial to apply prior to generating spatial heatmaps basic preprocessing 
routines to the data. The following shows how to normalize the count data, aggregate
replicates and then remove genes with unreliable expression responses.


<font color="red">[ThG-Comment 4: can you please substantially improve the text
in this preprocessing step as well as the help files of the corresponding
functions. I am not able to follow why and how certain parts are done. For
instance, why is ratio a normalization method?  The final result also doesn't
seem to be log ratios. You need to better describe what you are doing here.
Asking readers to look through your code is not appropriate for a package.]
</font>

Specifically, the following example eliminates genes with expression values larger 
