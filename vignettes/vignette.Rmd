...
of this information is provided by this package. Below this so called
'targets' file is imported and used to replace the longer version in the `colData` slot. 
It is important to note that for this well annotated data set 
this step is optional and mainly serves the purpose of shorting the text in some 
...
the `assay` slot of `rse.hum`. Since it contains raw RNA-Seq count data, it is
often beneficial to apply basic preprocessing routines prior to plotting
spatial heatmaps. The following shows how to normalize the count data, filter
the results and also collapse technical and/or biological replicates by
applying basic summary statistics such as mean, median or other. 

The following example eliminates genes with expression values larger than 5 in
at least 1% of all samples (pOA=c(0.01, 5)), and with coefficient of variance
