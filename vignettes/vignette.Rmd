...
any shape. For plotting spatial heatmaps, the Scalable Vector Graphics (SVG)
format has been chosen as image encoding since it is a flexible and widely
adapted vector graphics format that provides many advantages for
computationally embedding numerical information in images. SVG is based on XML
formatted text describing all lines, shapes and text in images.  In case of
biological images the shapes often represent anatomical or cell structures. To
assign colors to specific features in spatial heatmaps, _annotated SVG_ (aSVG)
files have to be used where the shapes of interest are labeled according to
certain conventions so that they can be colored programmatically. Raw SVGs and
aSVGs of anatomical structures can be downloaded from many sources including
the repositories mentioned above.  Alternatively, users can generate them
themselves with vector graphics software such as
[Inkscape](https://inkscape.org/). Typically, in aSVGs one or more shapes
making up a feature of interest, such as the cell shapes of an organ, are
grouped together by a common feature identifier. Via these identifiers one or
many feature types can be colored simultaneously in an aSVG according to
...
color gradient parameter. To interpret the meaning of the colors, the
corresponding color key can be included in the spatial heatmap plots.
Additional formatting details of both aSVG images and assay data are provided
in the [Supplement Section](#sup) of this vignette. 
