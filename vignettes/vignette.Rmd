---
title: "_spatialHeatmap_: Visualizing Cell-specific Gene Expression in Annatomical Maps Integrated with Multi-Gene Network Graphics"
...
and cell-specific gene expression values in form of colors plotted onto the
corresponding spatial features of an annatomical map. Via a user definable
color gradient, the gene expression values are represented by characteristic
colors and plotted onto the corresponding tissue or cell samples profiled in an
assay. This core functionality is called spatial heatmap. It is enhanced with
gene network visualization tools, including matrix heatmaps and network
representations of groups of genes (_e.g._ gene modules) sharing related
expression patterns. The functionalies of _spatialHeatmap_ can be used
either in a command-driven mode from within R or a graphical user interface (GUI)
provided by a Shiny App, also included in this package. While the former provides flexibility to customize and
automate analysis routines, the Shiny App includes a variety of convenience
features that will appeal to many biologists. The Shiny App has been designed
to work on both local systems as well as server-based deployments.

As annatomical images the package supports both tissue maps from public
repositories (_e.g._ EMBL-EBI's Expression Atlas) or custom images provided by
the user, as long as they are available in SVG (Scalable Vector Graphics)
format. The numeric values plotted onto a spatial heatmap are usually gene or
protein expression values from a wide range of expression profiling
technologies, such as microarray, RNA-Seq or proteomics experiments. For
convenience, several preprocessing and normalization methods are included that
support the usage of raw and/or preprocessed data. Currently, the main
application domains of the _spatialHeatmap_ package are numeric data sets and
spatially mapped images from biological and biomedical areas. However, the
package has been designed to also work with many other spatial data types, such
a population data plotted onto geographic maps. This level of flexitility is
one of the unique features of _spatialHeatmap_.  Existing software in this
...
customation functionalities by restricting users to utilizing pre-exisiting
expression data and/or fixed sets of anatomical image collections. To close this 
gap in the gene expression field, we have developed _spatialHeatmap_ as a generic 
R/Bioconductor package for plotting quantitative values onto any type of spatially 
mapped images in a programmable manner and/or in an intuitive to use GUI application.
...
layers tissues, several default and customizable options are provided to set 
transparency levels to maximize the exposure of the tissue layer of interest in 
the plots. This feature is highlighted below in the [mouse example](#mus_shm).

To maximize reusability and extensibility, the package organizes all numeric 
gene expression data along with the associated experimental design information in the widely used 
`SummarizedExperiment` container [@SummarizedExperiment]. Its `assays` slot contains 
the gene expression matrix with the rows and columns being the genes and tissue/conditions, 
respectively, while the `colData` slot contains replicate information. The tissues 
and/or cell types are spatially mapped into the corresponding SVG images. To visualize 
the data successfully, the expression data and SVG images have to meet certain 
[formatting requirements](#form), where the tissue identifiers in both the expression
object (`colData`) and the SVG images have to be compatible.

The following sections of this vignette highlight the most important
functionalities of this package. First, spatial heatmap plots are generated for
4 gene expression examples including both RNA-seq and microarray experiments
from [Human Brain](#hum), [Mouse Organs](#mus), [Chicken Organs](#chk), and
[Arabidopsis Shoots](#shoot). Second, additional visualization features
are introduced, which facilitate the visualization of gene modules sharing
similar expression patterns. This includes the visualization of hierarchical
clustering results with traditional matrix heatmaps ([Matrix Heatmap](#mhm)) 
as well co-expression network plots ([Network](#net)). Third, an overview of the 
corresponding [Shiny App](#shiny) is presented providing access to the same 
functionalities in interactive mode [@shiny; @shinydashboard]. Fourth, more 
advanced features for plotting customized spatial heatmaps are introduced using 
the Human Brain data set as an example.  
