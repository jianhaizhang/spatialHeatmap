...
## For better readability, enable scrollbar and turn off line wrapping in code/output chunks 
```{css, echo=FALSE}
pre code {
...
preprocessing steps are optional and can be skipped if needed. For instanse, fully preprocessed data can be applied to the function `spatial_hm` for [plotting spatial heatmaps](#hum_single) directly by skipping the preprocessing routines, but the sample column name and/or condition column name in the `colData` slot must be provided to `sam.factor` and/or `con.factor` respectively (Table \@ref(tab:arg)).  
^[I think one
should assure that skipping the preprocessing works seemlessly even without
...
`NULL` in the SHM plot of Figure \@ref(fig:musshm1).
