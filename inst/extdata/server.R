...
library(shiny); library(shinydashboard); library(grImport); library(rsvg); library(ggplot2); library(DT); library(gridExtra); library(ggdendro); library(WGCNA); library(Cairo); library(grid); library(XML); library(plotly); library(data.table); library(genefilter); library(flashClust); library(visNetwork); library(reshape2); library(igraph)
...
(input$fileIn=="organ_Mustroph") { svg.path <- "example/organ_final.svg"; svg.na <- "organ_final.svg" } else if (input$fileIn=="shoot_root_Mustroph") { svg.path <- "example/shoot_root_final.svg"; svg.na <- "shoot_root_final.svg" } else if (input$fileIn=="root_roottip_Mustroph") { svg.path <- "example/root_roottip_final.svg"; svg.na <- "root_roottip_final.svg" } else if (input$fileIn=="shoot_Mustroph") { svg.path <- "example/shoot_final.svg"; svg.na <- "shoot_final.svg" } else if (input$fileIn=="root_Geng") { svg.path <- "example/root_cross_final.svg"; svg.na <- "root_cross_final.svg" } else if (input$fileIn=="brain_Chen") { svg.path <- "example/brain_final.svg"; svg.na <- "brain_final.svg" } else if (input$fileIn=="map_Census") { svg.path <- "example/us_map_final.svg"; svg.na <- "us_map_final.svg" }
        # Make sure the style is correct. If the stroke width is not the same across polygons such as '0.0002px', '0.0216px', some stroke outlines cannot be recognised by 'PostScriptTrace'. Then some polygons are missing. Since the ggplot is based on 'stroke' not 'fill'.
        xmlfile <- xmlParse(svg.path); xmltop <- xmlRoot(xmlfile)
        ...
        }; svg.inter <- 'tmp/internal.svg'; saveXML(doc=xmlfile, file=svg.inter)
        ps.path <- sub(".svg$", ".ps", svg.inter); xml.path <- paste0(ps.path, ".xml")
        rsvg_ps(svg.inter, ps.path)
        PostScriptTrace(ps.path, xml.path) # Some tiny polygons (e.g. an unclosed short line) are misssing in this step, and this leads to corresponding coordinates lost. But the path ids extracted from svg xml are complete, so in the final spatial heatmaps, the colour of one tissue can jump to the first polygon in the next tissue (in lower order in Inkscape "XML Editor"). If place the tissue containing tiny polygons at the lowest in Inkscape "XML Editor", this problem will not be reflected in spatial heatmaps. These tiny polygons come from svg generated by GIMP, if they are removed, this problem can also be resoved.
	    grml <- xmlParse(xml.path); top <- xmlRoot(grml) # Use internal svg to get coordinates.
        # Match paths in svg with samples in matrix.
        xml <- xmlParse(svg.path) # Use original svg not internal svg to get path ids. Otherwise, errors can come up.
        xmltop <- xmlRoot(xml); size <- xmlSize(xmltop)
        do.call(file.remove, list(svg.inter, ps.path, xml.path))
        # Alternative way to get all ids in svg xml.
	    lis.ma <- xmlApply(xmltop[[size]], xmlAttrs) # Use "xmlApply" (returns list) not "xmlSApply" (returns matrix) to avoid unexpectations.
        ...
        id.xml1 <- NULL; for (i in seq_len(xmlSize(xmltop[[size]]))) {
          ...
          grp.path <- xmlApply(xmltop[[size]][[i]], xmlAttrs)
        ...
        for (i in seq_len((xmlSize(top)-1))) {
          ...
          if (xmlAttrs(top[[i]])['type']=="stroke") { # Switch from 'fill' to 'stroke' to avoid tiny-polygon issues. The all path ids including tiny polygons are extracted from raw 'svg' code. But some tiny polygons are missing in the 'xml' file resulting from 'PostScriptTrace', so the colors can shift in the final spatial heatmaps. E.g. a short line in original SVG image only shows up as a 'stroke' in the PostScriptTrace-xml file, even though 'fill' is set in the origianl SVG image. Therefore, if extract 'fill' this short line will miss and leads to color shift. A short line without 'stroke' is invisible in SVG image and also absent in the PostScriptTrace-xml file. As a result, it can also lead to color shift. 'Stroke' in the original SVG image is necessary.
      ...
      tis.path <- gsub("_\\d+$", "", id.xml1); #save(id.xml1, file="id.xml1"); #save(tis.path, file="tis.path")
  ...
  gs <- reactive({ 
    ...
    withProgress(message="Tissue heatmap: ", value=0, {
      ...
      if (!is.null(input$tis)) for (i in tis.df) { if (sub('_\\d+$', '', i) %in% input$tis) g.col[i] <- NA }
      g <- ggplot()+geom_polygon(data=g.df, aes(x=x, y=y, fill=tissue), color="black")+scale_fill_manual(values=g.col, guide=FALSE)+theme(axis.text=element_blank(), axis.ticks=element_blank(), panel.grid=element_blank(), panel.background=element_rect(fill="white", colour="grey80"), plot.margin=margin(0.1, 0.1, 0.1, 0.3, "cm"), axis.title.x=element_text(size=16,face="bold"), plot.title=element_text(hjust=0.5, size=20))+labs(x="", y="")+scale_y_continuous(expand=c(0.01, 0.01))+scale_x_continuous(expand=c(0.01, 0.01))+ggtitle(paste0(k, "_", j)); return(g)
     ...
     if (!is.null(con())) {
       ...
       png('tmp/delete.png'); grob <- lapply(g.lis, ggplotGrob); dev.off()
       # do.call(file.remove, list('tmp/delete.png'))
  ...
  gs <- reactive({ 
    ...
    withProgress(message="Spatial heatmap: ", value=0, {
      ...
      names(g.col) <- tis.df <- unique(g.df[, 'tissue'])
      # Make selected tissues transparent by setting their colours as NA.
      if (!is.null(input$tis)) for (i in tis.df) { if (sub('_\\d+$', '', i) %in% input$tis) g.col[i] <- NA }
      g <- ggplot()+geom_polygon(data=g.df, aes(x=x, y=y, fill=tissue), color="black")+scale_fill_manual(values=g.col, guide=FALSE)+theme(axis.text=element_blank(), axis.ticks=element_blank(), panel.grid=element_blank(), panel.background=element_rect(fill="white", colour="grey80"), plot.margin=margin(0.1, 0.1, 0.1, 0.3, "cm"), axis.title.x=element_text(size=16,face="bold"), plot.title=element_text(hjust=0.5, size=20))+labs(x="", y="")+scale_y_continuous(expand=c(0.01, 0.01))+scale_x_continuous(expand=c(0.01, 0.01))+ggtitle(paste0(k, "_", j)); return(g)
  ...
  visNet <- reactive({
    ...
    withProgress(message="Computing network:", value=0, {
      ...
      ann <- geneIn()[[2]]; if (!is.null(ann)) node <- data.frame(id=colnames(adj.m), value=colSums(adj.m), title=ann[colnames(adj.m), ], borderWidth=2, color.border="black", color.highlight.background="orange", color.highlight.border="darkred", color=NA, stringsAsFactors=FALSE)
