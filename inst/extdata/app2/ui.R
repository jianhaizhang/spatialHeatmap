...
shinyUI(dashboardPage(
  ...
  dashboardSidebar(
    ...
    sidebarMenu(
      ...
      radioButtons(inputId='cs.v', label='Colour scale based on:', choices=c("Selected genes"="sel.gen", "Whole
matrix"="w.mat"), selected="sel.gen", inline=TRUE), 
      ...
      selectInput("gen.sel","Select a gene to display matrix heatmap & network.", c("None"), selected="None", width=190),
  ...
  dashboardBody(
    ...
    tabItems(
      ...
      box(title="Spatial Heatmap Instruction", status="primary", solidHeader=TRUE, collapsible=TRUE, p(ins.mat, align="justify"), width=12),
      ...
      box(title="Expression Matrix", status="primary", solidHeader=TRUE, collapsible=TRUE, fluidRow(splitLayout(cellWidths=c("1%", "98%", "1%"), "", dataTableOutput("dt"), "")), height=NULL, width=12),
      ...
      box(title="Spatial Heatmap", status="primary", solidHeader=TRUE, collapsible=TRUE, fluidRow(splitLayout(cellWidths=c("1%", "7%", "91%", "1%"), "", plotOutput("bar"), plotOutput("sh"), "")), width=9, height=NULL),
