...
library(shiny); library(shinydashboard); library(grImport); library(rsvg); library(png); library(ggplot2); library(DT); library(gridExtra); library(ggdendro); library(WGCNA); library(Cairo); library(grid); library(XML); library(plotly); library(data.table); library(genefilter); library(flashClust); library(visNetwork)
...
shinyServer(function(input, output, session) {
  ...
  output$dt <- renderDataTable({
    ...
    filter="top", extensions='Scroller', options=list(pageLength=10, lengthMenu=c(5, 15, 20), autoWidth=TRUE, scrollCollapse=TRUE, deferRender=TRUE, scrollX=TRUE, scrollY=200, scroller=TRUE), class='cell-border strip hover') %>% formatStyle(0, backgroundColor="orange", cursor='pointer') %>% 
...
updateCheckboxGroupInput(session, "grp", label='', choices=c('NA', gen.sel), selected='NA', inline=T)
...
(input$fileIn=="organ_Mustroph") { svg.path <- "example/organ_final.svg"; svg.na <- "organ_final.svg" } else if (input$fileIn=="shoot_root_Mustroph") { svg.path <- "example/shoot_root_final.svg"; svg.na <- "shoot_root_final.svg" } else if (input$fileIn=="root_roottip_Mustroph") { svg.path <- "example/root_roottip_final.svg"; svg.na <- "root_roottip_final.svg" } else if (input$fileIn=="shoot_Mustroph") { svg.path <- "example/shoot_final.svg"; svg.na <- "shoot_final.svg" } else if (input$fileIn=="root_Geng") { svg.path <- "example/root_cross_final.svg"; svg.na <- "root_cross_final.svg" } else if (input$fileIn=="brain_Chen") { svg.path <- "example/brain_final.svg"; svg.na <- "brain_final.svg" } else if (input$fileIn=="map_Census") { svg.path <- "example/us_map_final.svg"; svg.na <- "us_map_final.svg" }
  ...
  # All tissues in SVG code.
  tis <- reactive({
    if (!input$fileIn=='Compute online') return()
	if (((input$fileIn=="Compute locally"|input$fileIn=="Compute online") & !is.null(input$svgInpath))|(grepl("_Mustroph$|_Geng$|_Chen$|_Census$", input$fileIn) & is.null(input$svgInpath))) {    
	
	xmlfile <- xmlParse(input$svgInpath$datapath); pol <- xmlRoot(xmlfile)['g']
    tis <- NULL; for (i in seq_len(xmlSize(pol))){

      for (j in seq_len(xmlSize(pol[[i]]))) {

        # Copy the referenced node and use it to replace the 'use' node. Keep the 'id' and 'title' of use node unchanged.
	nod <- pol[[i]][[j]]; if (xmlName(nod)=='use' & any(names(nod)=='title')) {
	  
          # Extract all attributes and title of the 'use' node.
	  t <- xmlClone(nod[['title']]); att <- xmlAttrs(nod); na <- names(att); val <- as.character(att); names(val) <- na 
	  # From the last node of whole SVG code.
          idx <- rev(seq_len(xmlSize(pol))); for (k in idx) {

	    ply <- pol[[idx[k]]]
            label <- NULL; for (l in seq_len(xmlSize(ply))) { label <- c(label, paste0('#', xmlGetAttr(ply[[l]], 'inkscape:label'))) }
            # Which node is referenced by 'use'?
            w <- which(label %in% xmlGetAttr(nod, 'xlink:href'))
	    if(length(w)==0) next
	    # Clone the referenced node and add original attributes and title to the cloned node.
	    pol[[i]][[j]] <- xmlClone(ply[[w]], recursive=TRUE)
	    addAttributes(pol[[i]][[j]], .attrs = val)
	    removeChildren(pol[[i]][[j]], kid='title')
	    addChildren(pol[[i]][[j]], kid=t)
            if (length(w)!=0) break

	  }

        }
        # Extract all tissues from 'title'.
        if (length(names(nod))!=0 & any(names(nod)=='title')) tis <- c(tis, xmlValue(nod[['title']]))

      }

    }; return(list(xmlfile=xmlfile, pol=pol, tis=tis))
  ...
  observe({
    ...
    input$fileIn; geneIn(); input$adj.modInpath; input$A; input$p; input$cv1; input$cv2; input$min.size; input$net.type
    updateCheckboxGroupInput(session, inputId="tis", label='All tissues in the image:', choices=c(tis()[['tis']]), selected='', inline=TRUE) })
  ...
  output$sh <- renderImage({ 

    if (is.null(gID$new)) return(NULL)
    withProgress(message="Tissue heatmap: ", value=0, {
     ...
     cname <- colnames(geneIn()[["gene2"]]); idx <- grep("__", cname); c.na <- cname[idx]; tis.col <- gsub("(.*)(__)(.*$)", "\\1", c.na)
     tis.path <- tis()[['tis']]; con <- con(); con.uni <- unique(con())
     xmlfile <- tis()[['xmlfile']]; pol <- tis()[['pol']]
     style1 <- 'fill:snow;fill-opacity:1;stroke:#000000;stroke-width:0.21602426;stroke-miterlimit:4;stroke-dasharray:none;stroke-opacity:1' 
     style2 <- ';fill-opacity:1;stroke:#000000;stroke-width:0.21602426;stroke-miterlimit:4;stroke-dasharray:none;stroke-opacity:1' 
    
      # Assgin colours to a specific row (a gene's expression values).
      svg.png <- function(gen, cdt) {

	if (gen=='none') col.r <- NULL else {

          col.r <- NULL; for (i in gene[gen, ]) {

            ab <- abs(i-geneV()); col.idx <- which(ab==min(ab))[1]; col.r <- c(col.r, color$col[col.idx])

          }

        }

       # Assign a colour to each tissue in the svg code.
       col.sel <- NULL; con.idx <- grep(paste0("^", cdt, "$"), con)
       tis.col1 <- tis.col[con.idx]; col.r1 <- col.r[con.idx]
       for (i in tis.path) {

         tis.idx <- which(tis.col1 %in% i); if (length(tis.idx)==1) { 
		  
           col0 <- col.r1[tis.idx]; names(col0) <- i; col.sel <- c(col.sel, col0)
          
         }
 
       }

       xml <- xmlClone(xmlfile, recursive=TRUE); pol.g <- xmlRoot(xml)['g']
       # 'use' duplicate.
       for (i in seq_len(xmlSize(pol.g))) {

         for (j in seq_len(xmlSize(pol.g[[i]]))) {

           nod <- pol.g[[i]][[j]]; if (xmlName(nod)!='a') {
    	       
	       # If the node (group or path) has a title, then it is target tissue.
	       if (length(names(nod))!=0 & any(names(nod)=='title')) { 
		 # Grab the corresponding color of the target tissue and assign it to style.
	        col.tis <- col.sel[xmlValue(nod[['title']])] 
                if (length(col.tis)!=0) if (!is.na(col.tis)) { sty <- paste0('fill:', col.tis, style2); addAttributes(nod, style=sty) } else { addAttributes(nod, style=style1);
		# Test transparency.
		if (xmlValue(nod[['title']])=='temporal lobe') addAttributes(nod, style="fill:#a016a2;stroke:#000000;fill-opacity:0;opacity:1;stroke-opacity:1") 
		}

               } else if (length(names(nod))==0) { addAttributes(nod, style=style1) }

             # Change style of paths inside a group.
             if (xmlName(nod)=='g') {
               
              sty <- xmlGetAttr(nod, 'style') 
              for (k in seq_len(xmlSize(nod))) { if (xmlName(nod[[k]])!='title') addAttributes(nod[[k]], style=sty) }
    ...
    tit <- paste0(gen, '_', cdt); img.s <- paste0(tit, '.svg'); saveXML(doc=xml, file=img.s)
    img.p <- paste0(tit, '.png')
    rsvg_png(svg=img.s, file=img.p, width=1500, height=1500) 
    return(list(tit=tit, p=readPNG(img.p)))
    
    }

    pngs <- tits <- NULL; for (i in gID$new) {
	
      for (j in con.uni) {

        lis <- svg.png(gen=i, cdt=j)
	    pngs <- c(pngs, lis['p']); tits <- c(tits, lis[['tit']])

      }

    }
    png('test.png', 1500, 1500)
    par(mfrow=c(3, 2), mar=c(0.1, 0.1, 1.5, 0.1))
    sapply(seq_along(pngs), function(i) {
      
      plot.new(); plot.window(xlim=c(0, 1), ylim=c(0, 1), asp=1)
      rasterImage(pngs[[i]], 0, 0, 1, 1); title(tits[[i]], font.main=2)
      
    }); dev.off()

    })

    list(src='test.png', contentType="image/png", alt = "This is alternate text")
