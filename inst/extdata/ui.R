...
ins.input3.4 <- "In the column names of sample/condition and meta data, only letters, digits, dots, single underscore, or single space are allowed. "
...
ins.input4 <- "The \"Compute locally\" option is designed for large gene expression matrix (e.g. > 10,000 genes), since gene network modules are identified using the R package WGCNA and the computation is intensive for large expression matrix. To maintain good performance on large matrices, this process is expected to be performed on user's computer. The instruction on how to compute locally is provided in the documentation of the function \"filter.data\" and \"adj.mod\" in the R package \"spatialHeatmap\"."

ins.input5 <- "The \"Compute online\" option is designed for small gene expression matrix (e.g. < 10,000 genes). The first two items filter genes according to a proportion of samples where a gene's expression values exceed a threthold A. Only the genes exceeding the proportion are maintained. The third and fourth items filter genes according to the coefficient of variation (CV). Only the genes with CV between the two specified values are kept. To save time, the app is designed to compute network modules only once when the matrix heatmap is displayed for the first time, since module assignments are save in memory. So displaying matrix heatmap of another gene will not trigger re-computation. But if the gene expression martix or filter parameters are changed, the modules are computated again."
...
shinyUI(dashboardPage(
  ...
  dashboardHeader(title="spatialHeatmap (updated: 2019-11-23)", 
  titleWidth=380),
  ...
  dashboardBody(
    ...
    tabItems(
      ...
      https://jianhaizhang.github.io/SVG_tutorial_file/SVG_tutorial.html>SVG tutorial</a>"), ".", align="justify"), p(ins.input2, align="justify"), p(ins.input3.1, strong(ins.input3.2), ins.input3.3, strong(ins.input3.4), ins.input3.5, HTML("&nbsp"), downloadButton("dld.svg", "Download SVG image"), downloadButton("dld.data", "Download gene expression matrix"), align="justify"), p(ins.input4, align="justify"), p(ins.input5, align="justify"), p(ins.input6, align="justify"), width=12), 
